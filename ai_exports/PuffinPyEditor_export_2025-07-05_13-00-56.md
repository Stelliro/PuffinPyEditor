...
## Export Timestamp: 2025-07-05T13:00:56.460770
---
...
class PluginManager:
    ...
    ESSENTIAL_PLUGIN_IDS = {
        'pythong_tools',  # Core script execution and output panel
    }
    ...
    def _resolve_dependencies(self) -> List[str]:
        ...
        while len(load_order) < len(self.plugins):
            ...
            for plugin_id in sorted(list(ready)):
                plugin = self.plugins.get(plugin_id)
                if not plugin: continue  # Should not happen, but for safety

                can_load = True

                # This section correctly handles version checks if dependencies are a dict
...
class PuffinPluginAPI:
    ...
    def add_dock_panel(self, area_str: str, widget, title: str, icon_name: Optional[str] = None):
        area_map = {
        ...
        self._main_window.add_dock_panel(widget, title, qt_area, icon_name)
        log.info(f"Plugin registered dock panel '{title}'")
    ...
    # Keep the old name for potential backwards compatibility if any other plugin uses it
    def register_dock_panel(self, content_widget, title, area, icon_name=None):
        ...
        self.add_dock_panel(list(area_map.keys())[list(area_map.values()).index(area)], content_widget, title,
                            icon_name)
...
class ThemeManager:
    ...
    def apply_theme_to_app(self, app: Optional[QApplication]):
        ...
        ca, su, sd = get_arrow_svg_uri('down', ac), get_arrow_svg_uri('up', ac), get_arrow_svg_uri('down', ac)
...
```python
# PuffinPyEditor/core_debug_tools/enhanced_exceptions/exception_dialog.py [MODIFIED]
import traceback
import platform
import sys
...
### File: `/plugins/basic_highlighters/__init__.py`
...
### File: `/plugins/basic_highlighters/html_syntax_highlighter.py`
...
### File: `/plugins/basic_highlighters/json_syntax_highlighter.py`
...
### File: `/plugins/basic_highlighters/plugin.json`

```json
{
    "id": "basic_highlighters",
    "name": "Basic Language Highlighters",
    "author": "Gramps's Refactoring",
    "version": "1.0.0",
    "description": "Provides basic syntax highlighting for various languages like JSON and Rust.",
    "entry_point": "plugin_main.py"
}
```

### File: `/plugins/basic_highlighters/plugin_main.py`

```python
# PuffinPyEditor/plugins/basic_highlighters/plugin_main.py
from app_core.puffin_api import PuffinPluginAPI
from utils.logger import log

# MODIFIED: Import directly from the plugin's package, no 'highlighters' sub-package.
from .json_syntax_highlighter import JsonSyntaxHighlighter
from .rust_syntax_highlighter import RustSyntaxHighlighter


class BasicHighlightersPlugin:
    """A single, data-driven plugin to register multiple simple syntax highlighters."""
    ...
    def __init__(self, puffin_api: PuffinPluginAPI):
        self.api = puffin_api
        self.register_all_highlighters()

    def register_all_highlighters(self):
        """Loops through the data map and registers each highlighter."""
        count = 0
        for extension, highlighter_class in self.HIGHLIGHTER_MAP.items():
            try:
                self.api.register_highlighter(extension, highlighter_class)
                count += 1
            except Exception as e:
                log.error(f"Failed to register highlighter for '{extension}': {e}")

        log.info(f"BasicHighlighters: Registered {count} syntax highlighters.")


def initialize(puffin_api: PuffinPluginAPI):
    """Entry point for the Basic Highlighters plugin."""
    return BasicHighlightersPlugin(puffin_api)
```

### File: `/plugins/basic_highlighters/rust_syntax_highlighter.py`
...
class RustSyntaxHighlighter(QSyntaxHighlighter):
    ...
    def initialize_formats_and_rules(self):
        ...
        self.highlighting_rules.extend([
            ...
            # Function definitions (simplified for stability)
    ...
    def _format_function_definition(self, match, formats):
        """Custom formatter to style `fn` as keyword and name as functionName."""
        self.setFormat(match.capturedStart(1), match.capturedLength(1), formats["keyword"])
        self.setFormat(match.capturedStart(2), match.capturedLength(2), formats["functionName"])
...
### File: `/plugins/cpp_tools/__init__.py`

```python

```

### File: `/plugins/cpp_tools/cpp_syntax_highlighter.py`
...
### File: `/plugins/cpp_tools/plugin.json`
...
```json
{
    ...
    "author": "PuffinPy Team",
    "version": "1.0.0",
    "description": "Adds syntax highlighting and compile/run support for C/C++ files.",
    "entry_point": "plugin_main.py",
    "dependencies": {
        "python_tools": ">=1.1.1"
    }
...
### File: `/plugins/cpp_tools/plugin_main.py`
...
class CppToolsPlugin(QObject):
    def __init__(self, puffin_api: PuffinPluginAPI):
        ...
        if not self.output_panel:
            log.error("C++ Tools: Could not find Output Panel. This plugin needs Pythong Tools.")
            return
    ...
    def _get_output_panel(self):
        runner_plugin = self.api.get_plugin_instance("pythong_tools")
    ...
    def _on_tab_changed(self, index):
        ...
        if index != -1:
            ...
            if isinstance(widget, EditorWidget):
                data = self.main_window.editor_tabs_data.get(widget)
                if data and (fp := data.get('filepath', '')):
                    is_cpp = fp.lower().endswith(('.cpp', '.c', '.cxx'))
...
### File: `/plugins/csharp_tools/__init__.py`

```python

```

### File: `/plugins/csharp_tools/csharp_syntax_highlighter.py`

```python
# PuffinPyEditor/plugins/csharp_tools/csharp_syntax_highlighter.py
...
### File: `/plugins/csharp_tools/plugin.json`
...
```json
{
    ...
    "author": "PuffinPy Team",
    "version": "1.0.0",
    "description": "Adds syntax highlighting and compile/run support for C# files.",
    "entry_point": "plugin_main.py",
    "dependencies": {
        "python_tools": ">=1.1.1"
    }
...
### File: `/plugins/csharp_tools/plugin_main.py`
...
class CSharpToolsPlugin(QObject):
    def __init__(self, puffin_api: PuffinPluginAPI):
        ...
        self.output_panel = self._get_output_panel()
        if not self.output_panel:
            log.error("C# Tools: Could not find Output Panel. This plugin depends on Pythong Tools.")
            return
        ...
        if index != -1:
            ...
            if isinstance(widget, EditorWidget):
                ...
                if data and (filepath := data.get('filepath', '')):
                    is_cs_file = filepath.lower().endswith('.cs')
...
class GitHubToolsPlugin:
    ...
    # MODIFIED: Corrected access to plugin instance
    def _get_sc_panel(self):
        # The plugin manager stores Plugin objects. Access the `instance` attribute.
        if sc_plugin := self.plugin_manager.plugins.get('source_control_ui'):
            if sc_plugin.is_loaded and hasattr(sc_plugin.instance, 'source_control_panel'):
                return sc_plugin.instance.source_control_panel
        return None
...
### File: `/plugins/javascript_tools/javascript_syntax_highlighter.py`
...
### File: `/plugins/javascript_tools/plugin.json`
...
```json
{
    ...
    "author": "PuffinPy Team",
    "version": "1.0.0",
    "description": "Adds syntax highlighting and script execution for JavaScript files (.js).",
    "entry_point": "plugin_main.py",
    "dependencies": {
        "python_tools": ">=1.1.1"
    }
...
### File: `/plugins/javascript_tools/plugin_main.py`
...
class JavaScriptToolsPlugin(QObject):
    ...
    def _on_tab_changed(self, index):
        ...
        if index != -1:
            ...
            if isinstance(widget, EditorWidget):
                data = self.main_window.editor_tabs_data.get(widget)
                if data and (filepath := data.get('filepath', '')):
                    is_js_file = filepath.lower().endswith('.js')
    ...
    def _run_script(self):
        ...
        if not isinstance(current_widget, EditorWidget):
             return
        ...
        if self.output_panel is None:
            # MODIFIED: Fix typo 'pythong_tools' -> 'python_tools'
            runner_plugin = self.api.get_plugin_instance("python_tools")
            if not runner_plugin:
                self.api.show_message("critical", "Dependency Missing",
                                      "The Python Tools plugin, which provides the Output panel, is not loaded.")
                return
...
# MODIFIED: Removed top-level import to prevent circular dependencies
# from ui.editor_widget import EditorWidget
from app_core.puffin_api import PuffinPluginAPI
from app_core.theme_manager import theme_manager
from app_core.settings_manager import settings_manager
...
class MarkdownFormattingToolbar(QWidget):
    """A stylish, floating toolbar for common Markdown formatting actions."""
    # Signals for each formatting action
    ...
    def update_theme(self):
        colors = theme_manager.current_theme_data.get('colors', {})
        bg = colors.get('menu.background', '#3a4145')
        border = colors.get('input.border', '#555555')
        accent = colors.get('accent', '#88c0d0')
        style = f"""
...
class MarkdownEditorWidget(QWidget):
    """
    A full-featured dual-pane Markdown editor widget, combining a rich source
    editor with a live preview pane.
    """
    content_changed = pyqtSignal()

    def __init__(self, puffin_api: PuffinPluginAPI, completion_manager, parent=None):
        super().__init__(parent)
        # --- MODIFICATION: Deferred import of EditorWidget ---
        from ui.editor_widget import EditorWidget
    ...
    def _setup_ui(self):
        ...
        # Editor (Left Pane)
        self.editor_widget = self.EditorWidgetClass(self.api, self.completion_manager, self)
    ...
    def _connect_signals(self):
        # Update preview after a short delay of inactivity
        self.update_timer = QTimer(self)
        self.update_timer.setSingleShot(True)
        self.update_timer.setInterval(250)  # ms
    ...
    def contextMenuEvent(self, event):
        # Show floating toolbar on right-click in editor area
    ...
    def _insert_code_block(self):
        cursor = self.editor_widget.text_area.textCursor()
        cursor.beginEditBlock()
        # Add newlines around for better block separation
        cursor.insertText("\n```python\n\n```\n")
    ...
    def _sync_scroll_from_editor(self):
        return self._sync_scroll_factory(self.editor_widget.text_area.verticalScrollBar(),
                                         self.viewer.verticalScrollBar())

    @property
    def _sync_scroll_from_viewer(self):
        return self._sync_scroll_factory(self.viewer.verticalScrollBar(),
                                         self.editor_widget.text_area.verticalScrollBar())
    ...
    def _render_preview(self):
        ...
        # Restore scroll position after content re-renders
        QTimer.singleShot(0, lambda: viewer_scroll.setValue(int(viewer_scroll.maximum() * old_pos_ratio)))

    def update_theme(self):
        """Applies colors and fonts from the current theme to the preview pane."""
        colors = theme_manager.current_theme_data.get('colors', {})
        ...
        # CSS Stylesheet for the QTextBrowser preview
        style_sheet = f"""
            h1, h2, h3, h4, h5, h6 {{
                color: {accent};
...
### File: `/plugins/python_tools/code_runner.py`
...
### File: `/plugins/python_tools/output_panel.py`
...
### File: `/plugins/python_tools/plugin.json`
...
### File: `/plugins/python_tools/plugin_main.py`

```python
# PuffinPyEditor/plugins/pythong_tools/plugin_main.py
import os
from PyQt6.QtWidgets import QMessageBox
...
class PythongToolsPlugin(QObject):
    def __init__(self, puffin_api: PuffinPluginAPI):
        super().__init__()
        self.api = puffin_api
        self.main_window = self.api.get_main_window()
    ...
    def _setup_highlighter(self):
        """Register the PythonSyntaxHighlighter for .py files."""
        self.api.register_highlighter('.py', PythonSyntaxHighlighter)
        self.api.log_info("Pythong Tools: Registered highlighter for .py files.")

    def _setup_ui(self):
        self.api.register_dock_panel(self.output_panel, "Output", Qt.DockWidgetArea.BottomDockWidgetArea,
                                     'fa5s.terminal')
        self.run_action = self.api.add_menu_action("run", "Run Script", self._run_script, "F5", 'fa5s.play')
        self.stop_action = self.api.add_menu_action("run", "Stop Script", self.code_runner.stop_script, "Ctrl+F5",
                                                    'fa5s.stop-circle')
    ...
    def _run_script(self):
        ...
        if not filepath:
            if QMessageBox.question(self.main_window, "Save Required", "File must be saved.",
    ...
    def run_specific_script(self, filepath: str):
        """Public method to allow other plugins to run a specific script."""
        if filepath and os.path.exists(filepath):
            self.output_panel.clear_output()
            self.run_action.setEnabled(False)
            self.stop_action.setEnabled(True)
...
def initialize(puffin_api: PuffinPluginAPI):
    return PythongToolsPlugin(puffin_api)
```

### File: `/plugins/python_tools/python_syntax_highlighter.py`

```python
# PuffinPyEditor/plugins/pythong_tools/python_syntax_highlighter.py
from typing import Dict, List, Tuple
from PyQt6.QtGui import QSyntaxHighlighter, QTextCharFormat, QColor, QFont
from PyQt6.QtCore import QRegularExpression
from app_core.theme_manager import theme_manager
...
class PythonSyntaxHighlighter(QSyntaxHighlighter):
    ...
    def __init__(self, parent_document):
        ...
        self.initialize_formats_and_rules()
        log.info("PythonSyntaxHighlighter initialized from plugin.")
...
class TerminalPlugin:
    ...
    def shutdown(self):
        """
        THE FIX: This method is called by the main window on exit to ensure
        the terminal's underlying shell process is terminated correctly.
        """
...
class TerminalWidget(QWidget):
    ...
    def stop_process(self):
        if self.process.state() == QProcess.ProcessState.Running:
            log.info("Attempting to terminate terminal process...")
            # First, try to terminate gracefully
            self.process.terminate()
            # Wait a short time for it to exit
            if not self.process.waitForFinished(1000):
                log.warning("Terminal process did not terminate gracefully, killing.")
                self.process.kill()
...
def show_project_context_menu(panel, position: QPoint, path: str, is_dir: bool):
    ...
    if is_valid_selection:
        ...
        if path.lower().endswith('.py') and not is_dir:
            ...
            if runner_plugin:
                menu.addAction(qta.icon('fa5s.play', color='#4CAF50'), "Run Script",
...
from PyQt6.QtWidgets import (QWidget, QVBoxLayout, QTreeWidget, QTreeWidgetItem, QInputDialog, QMessageBox,
                             QProxyStyle, QStyle, QApplication, QAbstractItemView, QStyleOptionViewItem, QToolButton,
                             QHBoxLayout)
...
class NoDrawProxyStyle(QProxyStyle):
    def drawPrimitive(self, element, option, painter, widget=None):
        if element == QStyle.PrimitiveElement.PE_IndicatorBranch: return
...
class StyledTreeView(QTreeWidget):
    def __init__(self, puffin_api, parent_view, parent=None):
        super().__init__(parent)
        self.puffin_api, self.parent_view, self.theme_manager, self.file_handler = puffin_api, parent_view, puffin_api.get_manager("theme"), puffin_api.get_manager("file_handler")
        self.setStyle(NoDrawProxyStyle(self.style())); self.setDragEnabled(True); self.setAcceptDrops(True); self.setDropIndicatorShown(True); self.setDragDropMode(QAbstractItemView.DragDropMode.InternalMove)

    def paintEvent(self, e):
        super().paintEvent(e);
        if self.topLevelItemCount() == 0: return
        p = QPainter(self.viewport()); p.setRenderHint(QPainter.RenderHint.Antialiasing); th = self.theme_manager.current_theme_data.get('colors', {}); p.setPen(QPen(QColor(th.get('accent', '#83c092')), 1)); p.drawLine(8, 0, 8, self.viewport().height())

    def drawBranches(self, painter, rect, index):
        item = self.itemFromIndex(index)
        if not item or not item.parent() or item.parent() == self.invisibleRootItem(): return
        theme, item_rect, indent = self.theme_manager.current_theme_data.get('colors', {}), self.visualRect(index), self.indentation(); accent_color = QColor(theme.get('accent', '#83c092')); base_offset, half_indent = 14, indent / 2.0
        painter.save(); painter.setRenderHint(QPainter.RenderHint.Antialiasing); depth, temp_item = -1, item
        while temp_item.parent() and temp_item.parent() != self.invisibleRootItem(): depth += 1; temp_item = temp_item.parent()
        if depth < 0: painter.restore(); return
        pen = QPen(accent_color, 1); painter.setPen(pen)
        for i in range(depth):
            ancestor = item; [ancestor := ancestor.parent() for _ in range(depth - i)]
            if ancestor.parent() and ancestor.parent().indexOfChild(ancestor) < ancestor.parent().childCount() - 1: painter.drawLine(QPointF(base_offset + (i * indent) + half_indent, item_rect.top()), QPointF(base_offset + (i * indent) + half_indent, item_rect.bottom()))
        is_folder, is_last_child = (item.childCount() > 0 or (item.childCount() == 1 and not item.child(0).text(0))), (item.parent().indexOfChild(item) == item.parent().childCount() - 1)
        pen.setWidthF(2.0 if is_folder else 1.0); painter.setPen(pen); expander_x, parent_guide_x = base_offset + (depth * indent) + (indent * 0.25), (base_offset + ((depth - 1) * indent) + half_indent) if depth > 0 else base_offset
        center_y, diagonal_start_y = item_rect.center().y(), item_rect.center().y() - 4.0
        if depth > 0: painter.drawLine(QPointF(parent_guide_x, item_rect.top()), QPointF(parent_guide_x, item_rect.bottom() if not is_last_child else diagonal_start_y))
        painter.drawLine(QPointF(parent_guide_x, diagonal_start_y), QPointF(expander_x, center_y))
        if is_folder: painter.setPen(QPen(accent_color, 1.2)); self._draw_expander_at(painter, QPointF(expander_x, center_y), item.isExpanded())
        painter.restore()

    def _draw_expander_at(self, painter, pos, is_open):
        arrow_size = 3.5; p1 = pos + QPointF(-arrow_size / 2, -arrow_size); p2 = pos + QPointF(arrow_size / 2, 0); p3 = pos + QPointF(-arrow_size / 2, arrow_size); painter.save(); painter.translate(pos); painter.rotate(90 if is_open else 0); painter.translate(-pos); painter.drawLine(p1, p2); painter.drawLine(p2, p3); painter.restore()
        
    def startDrag(self, actions):
        if (item := self.currentItem()) and (data := item.data(0, Qt.ItemDataRole.UserRole)) and (path := data.get('path')): mime = QMimeData(); mime.setData(TREE_ITEM_MIME_TYPE, path.encode('utf-8')); drag = QDrag(self); drag.setMimeData(mime); drag.exec(Qt.DropAction.MoveAction)
        
    def dragMoveEvent(self, e):
        if not (s_bytes := e.mimeData().data(TREE_ITEM_MIME_TYPE)): super().dragMoveEvent(e); return
        t_item = self.itemAt(e.position().toPoint())
        if not t_item or not (t_data := t_item.data(0, Qt.ItemDataRole.UserRole)): e.ignore(); return
        d_dir, s_path = t_data.get('path') if t_data.get('is_dir') else os.path.dirname(t_data.get('path')), s_bytes.data().decode('utf-8')
        if not (os.path.normpath(s_path) == os.path.normpath(d_dir) or (os.path.isdir(s_path) and os.path.normpath(d_dir).startswith(os.path.normpath(s_path) + os.sep))): e.acceptProposedAction()
        else: e.ignore()

    def dropEvent(self, e):
        if not (s_bytes := e.mimeData().data(TREE_ITEM_MIME_TYPE)): super().dropEvent(e); return
        s_path, t_item = s_bytes.data().decode('utf-8'), self.itemAt(e.position().toPoint());
        if not t_item: e.ignore(); return
        self.parent_view._perform_file_operation(self.file_handler.move_item, s_path, t_item.data(0, Qt.ItemDataRole.UserRole).get('path') if t_item.data(0, Qt.ItemDataRole.UserRole).get('is_dir') else os.path.dirname(t_item.data(0, Qt.ItemDataRole.UserRole).get('path'))); e.accept()
        
    def keyPressEvent(self, e):
        if e.key() == Qt.Key.Key_Delete and (item := self.currentItem()) and (data := item.data(0, Qt.ItemDataRole.UserRole)) and (path := data.get('path')): self.parent_view._action_delete(path); e.accept()
...
class FileSystemListView(QWidget):
    def __init__(self, puffin_api: PuffinPluginAPI, parent: None = None):
        super().__init__(parent)
        self.api = puffin_api; self.project_manager = self.api.get_manager("project"); self.file_handler = self.api.get_manager("file_handler"); self.theme_manager = self.api.get_manager("theme")
        self.icon_provider = CustomFileIconProvider(self.api); self.git_statuses = {}
        self.fs_watcher, self.watched_paths = QFileSystemWatcher(self), set(); self.fs_watcher.directoryChanged.connect(self._schedule_refresh); self.fs_watcher.fileChanged.connect(self._schedule_refresh)
        self._refresh_timer = QTimer(self); self._refresh_timer.setSingleShot(True); self._refresh_timer.setInterval(300); self._refresh_timer.timeout.connect(self.refresh)
        self._internal_change_flag = False
        
        layout = QVBoxLayout(self); layout.setContentsMargins(5, 5, 5, 5); layout.setSpacing(5)
        toolbar_layout = QHBoxLayout(); self.expand_button = QToolButton(); self.expand_button.setIcon(qta.icon('fa5s.expand-arrows-alt', color='gray')); self.expand_button.setToolTip("Expand All"); self.expand_button.clicked.connect(self.expand_all)
        self.collapse_button = QToolButton(); self.collapse_button.setIcon(qta.icon('fa5s.compress-arrows-alt', color='gray')); self.collapse_button.setToolTip("Collapse All"); self.collapse_button.clicked.connect(self.collapse_all)
        self.refresh_button = QToolButton(); self.refresh_button.setIcon(qta.icon('fa5s.sync-alt', color='gray')); self.refresh_button.setToolTip("Refresh"); self.refresh_button.clicked.connect(self.refresh)
        toolbar_layout.addStretch(); toolbar_layout.addWidget(self.expand_button); toolbar_layout.addWidget(self.collapse_button); toolbar_layout.addWidget(self.refresh_button)
        layout.addLayout(toolbar_layout)

        self.tree_widget = StyledTreeView(self.api, self); self.tree_widget.setHeaderHidden(True); self.tree_widget.setAlternatingRowColors(True)
        self.tree_widget.setContextMenuPolicy(Qt.ContextMenuPolicy.CustomContextMenu); self.tree_widget.setIndentation(14); layout.addWidget(self.tree_widget)
        
        self.tree_widget.itemExpanded.connect(self.on_item_expanded); self.tree_widget.itemCollapsed.connect(self.on_item_collapsed); self.tree_widget.itemDoubleClicked.connect(self.on_item_double_clicked); self.tree_widget.customContextMenuRequested.connect(self.show_context_menu)
        if git := self.api.get_manager("git"): git.git_success.connect(self.refresh)
        self.file_handler.item_created.connect(self._on_item_created); self.file_handler.item_renamed.connect(self._on_item_renamed); self.file_handler.item_deleted.connect(self._on_item_deleted)

    def expand_all(self): self.tree_widget.expandAll()
    def collapse_all(self): self.tree_widget.collapseAll()
    def get_expanded_paths(self): s = set(); self._get_expanded_children_recursive(self.tree_widget.invisibleRootItem(), s); return list(s)

    def _get_expanded_children_recursive(self, item, s):
        if item and item.isExpanded() and (d := item.data(0, Qt.ItemDataRole.UserRole)) and (p := d.get('path')):
            s.add(os.path.normpath(p)); [self._get_expanded_children_recursive(item.child(i), s) for i in range(item.childCount())]

    def _add_item_to_tree(self, parent_item, path):
        is_dir, name = os.path.isdir(path), os.path.basename(path)
        for i in range(parent_item.childCount()):
            if (child := parent_item.child(i)) and child.text(0) == name: return
        item = QTreeWidgetItem(parent_item); item.setText(0, name); item.setIcon(0, self.icon_provider.icon(QFileInfo(path))); item.setData(0, Qt.ItemDataRole.UserRole, {'path': path, 'is_dir': is_dir})
        if is_dir: item.addChild(QTreeWidgetItem()); self._add_to_watcher(path)
        parent_item.sortChildren(0, Qt.SortOrder.AscendingOrder)
        return item

    def _on_item_created(self, item_type, path):
        log.debug(f"Handling item creation for: {path}"); parent_dir = os.path.dirname(path)
        if not (parent_item := self._find_item_by_path(self.tree_widget.invisibleRootItem(), parent_dir)): log.error(f"Cannot add tree item: parent '{parent_dir}' not found in tree."); return
        is_populated = not (parent_item.childCount() == 1 and not parent_item.child(0).text(0));
        if not parent_item.isExpanded() or not is_populated: parent_item.setExpanded(True)
        else: self._add_item_to_tree(parent_item, path)
        QTimer.singleShot(150, lambda: self._select_and_scroll_to_path(path));
        if item_type == "file": self.api.get_main_window()._action_open_file(path)
        
    def _on_item_renamed(self, _, old, new): self.api.get_main_window()._on_file_renamed(old, new); self.refresh(); QTimer.singleShot(100, lambda: self._select_and_scroll_to_path(new))
    def _on_item_deleted(self, _, path):
        if (item := self._find_item_by_path(self.tree_widget.invisibleRootItem(), path)) and (parent := item.parent()): parent.removeChild(item)
    
    def on_item_double_clicked(self, item, col):
        if (data := item.data(0, Qt.ItemDataRole.UserRole)) and not data.get('is_dir') and (path := data.get('path')) and os.path.isfile(path): self.api.get_main_window()._action_open_file(path)
    
    def _schedule_refresh(self, path):
        if not self._internal_change_flag: log.debug(f"External FS event for: {path}. Scheduling refresh."); self._refresh_timer.start()

    def refresh(self):
        log.info("Refreshing file explorer.")
        # MODIFIED: Logic now trusts the saved state entirely for restores.
        expanded_paths = set(settings_manager.get("explorer_expanded_paths", []))
        
        current = (data.get('path') if (data := item.data(0, Qt.ItemDataRole.UserRole)) else None) if (item := self.tree_widget.currentItem()) else None
        
        if self.watched_paths: self.fs_watcher.removePaths(list(self.watched_paths))
        self.watched_paths.clear(); self.tree_widget.clear(); self.git_statuses.clear()
        
        for p in (paths := self.project_manager.get_open_projects()): self.git_statuses.update(get_git_statuses_for_root(p)); self._add_to_watcher(p)
        if not paths: return
        
        for p in paths:
            item = QTreeWidgetItem(self.tree_widget, [os.path.basename(p)]); item.setToolTip(0, p); item.setIcon(0, qta.icon('fa5s.folder-open', color=self.theme_manager.current_theme_data.get('colors', {}).get('accent')))
            item.setData(0, Qt.ItemDataRole.UserRole, {'path': p, 'is_dir': True, 'is_root': True}); item.addChild(QTreeWidgetItem())
            
        self._set_expanded_children(self.tree_widget.invisibleRootItem(), expanded_paths)
        if current: self._select_and_scroll_to_path(current)
        get_git_statuses_for_root.cache_clear()

    # MODIFIED: Logic made recursive to handle nested expansions correctly.
    def _set_expanded_children(self, parent_item, expanded_set):
        ...
        for i in range(parent_item.childCount()):
            ...
            if path and os.path.normpath(path) in expanded_set:
                child.setExpanded(True) # This triggers on_item_expanded, which populates the children
                # Now that the children are populated, we can recurse into them.
                # Use QTimer to ensure the expansion has processed before recursing.
                QTimer.singleShot(0, lambda c=child, s=expanded_set: self._set_expanded_children(c, s))
                
    def _add_to_watcher(self, path):
        if path and path not in self.watched_paths: self.fs_watcher.addPath(path); self.watched_paths.add(path)
    def _remove_from_watcher(self, path):
        if path and path in self.watched_paths: self.fs_watcher.removePath(path); self.watched_paths.remove(path)
    def _select_and_scroll_to_path(self, path):
        if item := self._find_item_by_path(self.tree_widget.invisibleRootItem(), path):
            self.tree_widget.setCurrentItem(item); self.tree_widget.scrollToItem(item, QAbstractItemView.ScrollHint.PositionAtCenter)

    def _find_item_by_path(self, parent, p_find):
        norm_p = os.path.normpath(p_find)
        for i in range(parent.childCount()):
            if (c := parent.child(i)) and (d := c.data(0, Qt.ItemDataRole.UserRole)) and os.path.normpath(d.get('path', '')) == norm_p: return c
            if c and c.isExpanded() and (found := self._find_item_by_path(c, p_find)): return found
        return None
        
    def _populate_node(self, parent_item, path, preserve=False):
        if not os.path.isdir(path): return
        expanded = {c.data(0, Qt.ItemDataRole.UserRole).get('path') for i in range(parent_item.childCount()) if (c := parent_item.child(i)) and c.isExpanded() and c.data(0, Qt.ItemDataRole.UserRole)} if preserve else set()
        parent_item.takeChildren()
        try:
            colors, proj_root = self.theme_manager.current_theme_data.get('colors', {}), self.project_manager.get_active_project_path()
            if proj_root: self.git_statuses = get_git_statuses_for_root(proj_root); get_git_statuses_for_root.cache_clear()
            u, m, i_color = QColor(colors.get('git.added', '#a7c080')), QColor(colors.get('git.status.foreground', '#87ceeb')), QColor(colors.get('syntax.comment', '#5f6c6d'))
            for e in sorted(os.scandir(path), key=lambda x: (not x.is_dir(), x.name.lower())):
                if e.name.startswith(('__', '.')) and e.name != '.gitignore': continue
                item = QTreeWidgetItem(parent_item, [e.name]); item.setIcon(0, self.icon_provider.icon(QFileInfo(e.path))); item.setData(0, Qt.ItemDataRole.UserRole, {'path': e.path, 'is_dir': e.is_dir()})
                if s := self.git_statuses.get(os.path.normpath(e.path)):
                    if s == '??': item.setForeground(0, u)
                    elif 'M' in s: item.setForeground(0, m)
                    elif s == '!!': item.setForeground(0, i_color)
                if e.is_dir(): item.addChild(QTreeWidgetItem()); self._add_to_watcher(e.path)
                if preserve and e.path in expanded: item.setExpanded(True)
        except OSError as e: log.warning(f"Could not scan directory: {path} - {e}")

    def on_item_expanded(self, item):
        if (d := item.data(0, Qt.ItemDataRole.UserRole)) and (p := d.get('path')) and item.childCount() == 1 and not item.child(0).text(0): self._populate_node(item, p, True)
    
    def on_item_collapsed(self, item):
        if (d := item.data(0, Qt.ItemDataRole.UserRole)) and not d.get('is_root', False):
            item.takeChildren(); item.addChild(QTreeWidgetItem())
            if p := d.get('path'): self._remove_from_watcher(p)
            
    def show_context_menu(self, position):
        item = self.tree_widget.itemAt(position); path = (item.data(0, Qt.ItemDataRole.UserRole)['path'] if item and item.data(0, Qt.ItemDataRole.UserRole) else self.project_manager.get_active_project_path())
        if not path and self.project_manager.is_project_open(): path = self.project_manager.get_active_project_path()
        if not path: return
    ...
    def _perform_file_operation(self, operation, *args):
        self._internal_change_flag = True
        try:
            success, message = operation(*args)
            if not success: QMessageBox.critical(self, "Operation Failed", message)
        finally:
            self._internal_change_flag = False
            
    def _action_new_file(self, target_dir):
        fn, ok = QInputDialog.getText(self, "New File", "Enter file name:");
        if ok and fn: self._perform_file_operation(self.file_handler.create_file, os.path.join(target_dir, fn))
    
    def _action_new_folder(self, target_dir):
        fn, ok = QInputDialog.getText(self, "New Folder", "Enter folder name:");
        if ok and fn: self._perform_file_operation(self.file_handler.create_folder, os.path.join(target_dir, fn))
    
    def _action_rename(self, old_path):
        item = os.path.basename(old_path); new, ok = QInputDialog.getText(self, f"Rename '{item}'", "New name:", text=item)
        if ok and new and new != item: self._perform_file_operation(self.file_handler.rename_item, old_path, new)
        
    def _action_delete(self, p_del):
        if not p_del: return
        name, is_dir, itype = os.path.basename(p_del), os.path.isdir(p_del), "folder" if os.path.isdir(p_del) else "file"
        msg = f"Are you sure you want to permanently delete this {itype}?\n\n'{name}'"
        try:
            if is_dir and any(os.scandir(p_del)): msg += "\n\n<b>Warning: The folder is not empty.</b>"
        except OSError: pass
        if QMessageBox.question(self, f"Confirm Delete", msg, QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No, QMessageBox.StandardButton.No) == QMessageBox.StandardButton.Yes: self._perform_file_operation(self.file_handler.delete_item, p_del)
...
(Version 7.9 - Gutter and Highlight Theming Fix)
"""
from __future__ import annotations
from typing import Optional, Set, Dict
from math import cos, sin

from PyQt6.QtWidgets import (QWidget, QPlainTextEdit, QTextEdit, QHBoxLayout,
                             QVBoxLayout)
...
THEME = {
    'gutter.highlight.background': ('editorGutter.highlightBackground', '#d68c5840'),
    'gutter.hover.background': ('editorGutter.hoverBackground', '#ffffff1a'),
    'editor.background': ('editor.background', '#1a1b26'),
    'editor.foreground': ('editor.foreground', '#c0caf5'),
    'selection.background': ('editor.selectionBackground', '#2e3c60'),
    'line.highlight': ('editor.lineHighlightBackground', '#222436'),
    'plugin.highlight': ('editor.wordHighlightBackground', '#d68c5840'),
    'gutter.background': ('editorGutter.background', '#16161e'),
    'gutter.foreground': ('editorLineNumber.foreground', '#545c7e'),
    'gutter.active.foreground': ('editorLineNumber.activeForeground', '#a9b1d6'),
    'gutter.ruler': ('editorGutter.ruler.color', '#41a6b5'),
    'indent.guide': ('editorIndentGuide.background', '#3b4261'),
}
...
class HighlightManager(QObject):
    ...
    def clear_highlights(self, source_id: str):
        if source_id in self._highlights: del self._highlights[source_id]; self.highlights_changed.emit()

    def get_all_highlights(self) -> Dict[int, QColor]:
        combined = {};
        [combined.update(s) for s in self._highlights.values()];
        return combined
...
class GutterWidget(QWidget):
    ...
    def mouseMoveEvent(self, event: QMouseEvent):
        line_num = self.editor.cursorForPosition(event.pos()).blockNumber()
...
class MiniMapWidget(QWidget):
    def __init__(self, editor: 'CodeEditor'):
        ...
        # Connect signals to update minimap when editor view changes
    ...
    def paintEvent(self, event: QPaintEvent):
        ...
        painter.fillRect(event.rect(), QColor(colors.get('editorGutter.background', '#16161e')))
        ...
        line_height_in_map = 2
        total_content_height = total_lines * line_height_in_map
        scale = min(1.0, visible_height / total_content_height if total_content_height > 0 else 1.0)

        v_scroll = self.editor.verticalScrollBar()
        scroll_proportion = v_scroll.value() / v_scroll.maximum() if v_scroll.maximum() > 0 else 0
        scroll_offset_y = -(
                    total_content_height - visible_height) * scroll_proportion if total_content_height > visible_height else 0
        ...
        for i in range(total_lines):
            ...
            indent_level = len(block.text()) - len(text)

            y_pos = int(scroll_offset_y + (i * line_height_in_map) * scale)
            rect_h = max(1, int(line_height_in_map * scale))

            if y_pos > visible_height: break
            if text: painter.drawRect(QRectF(indent_level * 1.0, y_pos, len(text) * 1.0, rect_h))
        ...
        viewport_y = int(scroll_offset_y + (first_visible * line_height_in_map) * scale)
        viewport_h = max(1, int(visible_blocks * line_height_in_map * scale))
    ...
    def mousePressEvent(self, e):
        self._scroll_from_mouse(e.pos())

    def mouseMoveEvent(self, e):
        if e.buttons() & Qt.MouseButton.LeftButton: self._scroll_from_mouse(e.pos())

    def _scroll_from_mouse(self, pos):
        v_scroll = self.editor.verticalScrollBar()
...
class CodeEditor(QPlainTextEdit):
    def __init__(self, parent_container: 'EditorWidget', highlight_manager: HighlightManager):
        super().__init__(parent_container)
        self.container, self.highlight_manager = parent_container, highlight_manager
        self.highlight_manager.highlights_changed.connect(self.update_extra_selections)
        self.cursorPositionChanged.connect(self.update_extra_selections)

    def mousePressEvent(self, event: QMouseEvent):
        if event.pos().x() < self.container.gutter_widget.width():
            self.container.gutter_widget.mousePressEvent(event)
        else:
            super().mousePressEvent(event)

    def keyPressEvent(self, event: QKeyEvent):
        if (event.modifiers() & Qt.KeyboardModifier.ControlModifier) and (
                event.key() == Qt.Key.Key_Slash): self._toggle_comment(); event.accept(); return
    ...
    def paintEvent(self, event: QPaintEvent):
        # First, draw our custom background highlights
        painter = QPainter(self.viewport())
        paint_event_rect_f = QRectF(event.rect())

        if self.highlight_manager:
            for line_number, color in self.highlight_manager.get_all_highlights().items():
                block = self.document().findBlockByNumber(line_number - 1)
                if not (block.isValid() and block.isVisible()):
                    continue

                block_rect = self.blockBoundingGeometry(block).translated(self.contentOffset())
                if block_rect.intersects(paint_event_rect_f):
                    layout = block.layout()
                    if layout:
                        for i in range(layout.lineCount()):
                            line = layout.lineAt(i)
                            line_rect = QRectF(
                                0, block_rect.top() + line.rect().top(),
                                self.viewport().width(), line.rect().height()
                            )
                            # CRASH FIX: Convert event.rect() from QRect to QRectF for comparison
                            if line_rect.intersects(QRectF(event.rect())):
                                painter.fillRect(line_rect, color)
        
        # After painting backgrounds, let the default paint event handle text, selections, etc.
        super().paintEvent(event)
        
        # Finally, paint our indentation guides on top of everything.
        self.paint_indentation_guides(event)

    def handle_gutter_left_click(self, pos: QPointF):
        line_num = self.cursorForPosition(pos).blockNumber() + 1
        source_id = "gutter_click_highlights"
        manager = self.highlight_manager

        source_highlights = manager._highlights.get(source_id, {})
        if line_num in source_highlights:
            del source_highlights[line_num]
            if not source_highlights and source_id in manager._highlights:
                del manager._highlights[source_id]
            manager.highlights_changed.emit()
        else:
            colors = self.container.theme_manager.current_theme_data.get('colors', {})
            # Use the theme color. Fallback to a neutral, transparent gray just in case.
            highlight_color = QColor(colors.get('editor.userHighlightBackground', '#cccccc80'))
            manager.add_highlight(source_id, line_num, highlight_color)

    def set_line_spacing(self, m=1.4):
        c = QTextCursor(self.document());
        c.select(QTextCursor.SelectionType.Document);
        bf = c.blockFormat();
        bf.setLineHeight(m * 100, QTextBlockFormat.ProportionalHeight.value);
        c.setBlockFormat(bf);
        c.clearSelection();
    ...
    def update_extra_selections(self):
        ...
        if self.hasFocus():
            ...
            sel.format.setBackground(QColor(colors.get(*THEME['line.highlight'])))
    ...
    def paint_gutter_event(self, e: QPaintEvent, widget: GutterWidget):
        painter = QPainter(widget);
        colors = self.container.theme_manager.current_theme_data.get('colors', {});
        painter.fillRect(e.rect(), QColor(colors.get(*THEME['gutter.background'])))
        block = self.firstVisibleBlock();
        if not block.isValid(): return
        hls, top, bn = self.highlight_manager.get_all_highlights(), int(
            self.blockBoundingGeometry(block).translated(self.contentOffset()).top()), block.blockNumber()

        while block.isValid() and top <= e.rect().bottom():
            if block.isVisible():
                bh = int(self.blockBoundingRect(block).height())
                if top < e.rect().bottom() and (top + bh) > e.rect().top():
                    cur, hov = (self.textCursor().blockNumber() == bn), (bn == widget.hovered_line)

                    if (color := hls.get(bn + 1)):
                        painter.fillRect(QRect(0, top, widget.width(), bh), color)

                    if hov and not ((bn + 1) in hls):
                        painter.fillRect(QRect(0, top, widget.width(), bh), QColor(
                            colors.get(*THEME['gutter.hover.background'])))

                    if cur and self.hasFocus():
                        painter.fillRect(QRectF(0, top, widget.RULER_WIDTH, bh),
                                         QColor(colors.get(*THEME['gutter.ruler'])))

                    x, wi = widget.RULER_WIDTH, widget.width() - widget.RULER_WIDTH - 5
                    font, ck = self.font(), THEME['gutter.active.foreground'] if cur and self.hasFocus() else THEME['gutter.foreground']
                    painter.setFont(font);
                    painter.setPen(QColor(colors.get(*ck)))
                    painter.drawText(QRect(x, top, wi, bh), Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter,
                                     str(bn + 1))

            top += int(self.blockBoundingRect(block).height())
            block, bn = block.next(), bn + 1

    def paint_indentation_guides(self, event: QPaintEvent):
        painter = QPainter(self.viewport());
        colors = self.container.theme_manager.current_theme_data.get('colors', {});
        pen = QPen(QColor(colors.get(*THEME['indent.guide'])));
        pen.setStyle(Qt.PenStyle.DotLine);
        painter.setPen(pen)
        iw = self.container.settings.get("indent_width", 4)
        if iw <= 0: return
        tw = self.fontMetrics().horizontalAdvance(' ' * iw);
        offset, block = self.contentOffset(), self.firstVisibleBlock()
        while block.isValid() and block.isVisible():
            t, ind = block.text(), len(block.text()) - len(block.text().lstrip(' '))
            geom = self.blockBoundingGeometry(block)
            for i in range(1, int(ind / iw) + 1):
                x = (i * tw) + offset.x() - 1
    ...
    def _auto_indent(self):
        c, t = self.textCursor(), self.textCursor().block().text();c.insertBlock();c.insertText(
            t[:len(t) - len(t.lstrip())])

    def _toggle_comment(self):
        c, d, s, e = self.textCursor(), self.document(), c.selectionStart(), c.selectionEnd();
        sb, eb = d.findBlock(s), d.findBlock(e)
        if c.position() == e and c.columnNumber() == 0 and s != e: eb = eb.previous()
        bs, b = [], sb
        while True:
            bs.append(b)
            if not b.isValid() or b == eb: break
            b = b.next()
        ac = all(b.text().lstrip().startswith('#') for b in bs if b.text());
        c.beginEditBlock()
        for b in bs:
            bc, t = QTextCursor(b), b.text()
            if ac:
                if t.lstrip().startswith('# '):
                    ...
                    continue
                bc.setPosition(idx);
                bc.setPosition(idx + le, QTextCursor.MoveMode.KeepAnchor);
                bc.removeSelectedText()
            else:
                bc.movePosition(QTextCursor.MoveOperation.StartOfLine);bc.insertText('# ')
        c.endEditBlock()


class EditorWidget(QWidget):
    content_possibly_changed, cursor_position_display_updated = pyqtSignal(), pyqtSignal(int, int)
    status_message_requested = pyqtSignal(str, int)

    def __init__(self, puffin_api, completion_manager, highlight_manager: HighlightManager, parent=None):
        super().__init__(parent);
        self.puffin_api, self.theme_manager, self.settings = puffin_api, theme_manager, settings_manager
        self.highlight_manager = highlight_manager;
        self.filepath, self.highlighter = None, None
        self.find_panel, self.text_area = FindPanel(self), CodeEditor(self, highlight_manager)
        self.gutter_widget, self.minimap_widget = GutterWidget(self.text_area), MiniMapWidget(self.text_area)
        self.find_panel.hide()

        el = QHBoxLayout();
        el.setContentsMargins(0, 0, 0, 0);
        el.setSpacing(0);
        el.addWidget(self.text_area);
        el.addWidget(self.minimap_widget)
        ec = QWidget();
        ec.setLayout(el)
        self.main_layout = QVBoxLayout(self);
        ...
        self.text_area.textChanged.connect(self.minimap_widget.update)
        self.text_area.verticalScrollBar().valueChanged.connect(self.minimap_widget.update);
        self.highlight_manager.highlights_changed.connect(self.text_area.viewport().update)
        self.highlight_manager.highlights_changed.connect(self.text_area.update_extra_selections)
    ...
    def _update_widget_geometries(self):
        w = self.text_area.calculate_gutter_width();self.text_area.setViewportMargins(w, 0, 0, 0);cr = self.text_area.contentsRect();self.gutter_widget.setGeometry(
    ...
    def apply_styles_and_settings(self):
        f = QFont(self.settings.get("font_family"), self.settings.get("font_size"));
        self.text_area.setFont(f);
        self.text_area.setTabStopDistance(
            self.text_area.fontMetrics().horizontalAdvance(' ') * self.settings.get("indent_width"));
        self.text_area.set_line_spacing();
        self.update_theme()

    def update_theme(self):
        c = self.theme_manager.current_theme_data.get("colors", {});
        ss = f"QPlainTextEdit{{background-color:{c.get(*THEME['editor.background'])};color:{c.get(*THEME['editor.foreground'])};border:none;selection-background-color:{c.get(*THEME['selection.background'])};}}"
        self.text_area.setStyleSheet(ss)
    ...
    def get_cursor_position(self) -> tuple[int, int]:
        c = self.text_area.textCursor();return c.blockNumber() + 1, c.columnNumber() + 1

    def goto_line_and_column(self, l, co):
        pass

    def set_text(self, t: str):
        self.text_area.setPlainText(t);self.text_area.set_line_spacing()

    def get_text(self) -> str:
        return self.text_area.toPlainText()

    def set_highlighter(self, h):
        if self.highlighter: self.highlighter.setDocument(None)
        if h: self.highlighter = h(self.text_area.document())
    ...
    def show_find_panel(self):
        self.find_panel.show();self.find_panel.connect_editor(self)

    def hide_find_panel(self):
        self.find_panel.hide();self.text_area.setFocus()
```
...
class MainWindow(QMainWindow):
    ...
    def _integrate_linter_ui(self):
        ...
        self.add_dock_panel(self.problems_panel, "Problems", Qt.DockWidgetArea.BottomDockWidgetArea, "fa5s.bug")
    ...
    def eventFilter(self, obj, event):
        if obj is self and event.type() in (QEvent.Type.DragEnter, QEvent.Type.Drop) and event.mimeData().hasUrls():
            event.acceptProposedAction()
            if event.type() == QEvent.Type.Drop:
                for url in event.mimeData().urls():
                    if url.isLocalFile(): self._action_open_file(url.toLocalFile())
            return True
    ...
    def _create_core_widgets(self):
        ...
        btn = QToolButton()
        btn.setIcon(qta.icon('fa5s.plus'))
    ...
    def _create_core_actions(self):
        actions_map = {
            ...
            # Note: some actions have fewer properties
            ...
            "find_replace": ("&Find/Replace...", self.toggle_find_panel, "Ctrl+F", "fa5s.search"),
            "preferences": ("&Preferences...", self._action_open_preferences, "Ctrl+,", 'fa5s.cog'),
            "exit": ("E&xit", self.close, "Ctrl+Q", None),
            "force_quit": ("&Force Quit", self._action_force_quit, "Ctrl+Shift+Q", 'fa5s.exclamation-triangle')
        }
    ...
    def _create_toolbar(self):
        ...
        tb.addSeparator()
        self.toolbar_find_action = QAction(qta.icon('fa5s.search'), "Find/Replace", self);
        ...
        tb.addAction(self.toolbar_find_action);
        sp = QWidget();
    ...
    def _create_layout(self):
        lay = QHBoxLayout(self.central_widget); lay.setContentsMargins(0, 0, 0, 0); lay.addWidget(self.tab_widget)

    def _create_statusbar(self):
        self.setStatusBar(QStatusBar(self)); self.cursor_label = QLabel(
            " Ln 1, Col 1 "); self.statusBar().addPermanentWidget(self.cursor_label)
    ...
    def _apply_theme_and_icons(self, theme_id):
        ...
        self.toolbar_find_action.setIcon(qta.icon('fa5s.search'));
        c = self.theme_manager.current_theme_data.get('colors', {})
    ...
    def _on_theme_selected(self, theme_id):
        self.settings.set("last_theme_id", theme_id); self._apply_theme_and_icons(theme_id)
    ...
    def _action_save_file(self, ed=None, save_as=False):
        editor = ed or self.tab_widget.currentWidget()
        if not (isinstance(editor, EditorWidget) and (data := self.editor_tabs_data.get(editor))): return
        ...
        if new_fp := self.file_handler.save_file_content(data['filepath'], editor.get_text(), save_as):
            self.file_handler._add_to_recent_files(new_fp)
            data.update({'filepath': new_fp, 'original_hash': hash(editor.get_text())});
    ...
    def _action_save_all(self):
        if c := sum(1 for i in range(self.tab_widget.count()) if
                    self._is_editor_modified(w := self.tab_widget.widget(i)) and self._action_save_file(w)):
            self.statusBar().showMessage(f"Saved {c} file(s).", 3000)
    ...
    def _close_widget_safely(self, widget, event):
        if not isinstance(widget, EditorWidget):
            event.accept()
            if widget and widget.parent() is None: widget.deleteLater()
            return
        if self._is_editor_modified(widget):
            ret = QMessageBox.question(self, "Save Changes?",
                                       f"Save changes to {self.tab_widget.tabText(self.tab_widget.indexOf(widget)).replace(' *', '')}?",
                                       QMessageBox.StandardButton.Save | QMessageBox.StandardButton.Discard | QMessageBox.StandardButton.Cancel)
            if ret == QMessageBox.StandardButton.Cancel or (
                    ret == QMessageBox.StandardButton.Save and not self._action_save_file(
                widget)): event.ignore(); return
    ...
    def _action_force_quit(self):
        log.warning("Force Quit triggered."); QApplication.instance().quit()
    ...
    def _shutdown_plugins(self):
        log.info("Shutting down plugins..."); [p.instance.shutdown() for p in self.plugin_manager.get_loaded_plugins()
                                               if hasattr(p.instance, 'shutdown')]

    def _shutdown_managers(self):
        log.info("Shutting down core managers..."); [m.shutdown() for m in
    ...
    def closeEvent(self, e: QCloseEvent):
        ...
        settings_manager.set("open_files", open_fps, False)
        if hasattr(self, 'explorer_panel'): settings_manager.set("explorer_expanded_paths",
                                                                 self.explorer_panel.get_expanded_paths(), False)
        while self.tab_widget.count() > 0:
            dummy = QCloseEvent();
            self._close_widget_safely(self.tab_widget.widget(0), dummy)
            if not dummy.isAccepted(): self._is_app_closing = False; e.ignore(); return
            if (idx := self.tab_widget.indexOf(self.tab_widget.widget(0))) != -1: self.tab_widget.removeTab(idx)
    ...
    def _generate_header_line(self, file_path: str):
        ext, start = os.path.splitext(file_path)[1].lower(), self.COMMENT_MAP.get(
            os.path.splitext(file_path)[1].lower())
        if not start or not (root := self.project_manager.get_active_project_path()) or not file_path.startswith(
            root): return None
        return f"{start} PuffinPyEditor/{os.path.basename(root)}/{os.path.relpath(file_path, root).replace(os.sep, '/')} {self.END_COMMENT_MAP.get(start, '')}\n"

    def _add_header_to_new_file(self, file_path):
        if not (header := self._generate_header_line(file_path)): return
        try:
            with open(file_path, 'w', encoding='utf-8') as f:
                f.write(header); log.info(f"Autopopulated header for {file_path}")
        except IOError as e:
    ...
    def _update_file_header(self, file_path):
        ...
        try:
            ...
            with open(file_path, 'w', encoding='utf-8') as f:
                f.writelines(lines); log.info(f"Updated header for: {file_path}")
        except Exception as e:
