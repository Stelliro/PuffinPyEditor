...
## Export Timestamp: 2025-07-05T22:33:21.292390
---
...
REM THE FIX: Explicitly set the distpath to ensure the main EXE goes to the correct folder.
call pyinstaller !MAIN_SPEC! --noconfirm --distpath "!FINAL_DIR!"
...
REM THE FIX: Explicitly set the output file path via command line to avoid ambiguity.
set "INSTALLER_OUTPUT_FILE=dist\PuffinPyEditor_v!APP_VERSION!_Setup.exe"
call "!MAKENSIS_CMD!" "/DVERSION=!APP_VERSION!" "/XOutFile !INSTALLER_OUTPUT_FILE!" "!INSTALLER_SCRIPT!"
if errorlevel 1 exit /b 1
...
class GitHubToolsPlugin:
    ...
    def _run_build_script(self, project_path):
        ...
        if not os.path.exists(build_script): self._on_release_step_failed(
            f"Build script not found at '{build_script}'."); return
        self.build_output_buffer.clear();
        args = []
        if self.api.get_manager("settings").get("cleanup_after_build", True): args.append("cleanup")
        version_str = self._release_state['dialog_data']['tag'].lstrip('v')
        args.extend(["--version", version_str])
        if nsis_path := self.api.get_manager("settings").get("nsis_path"): args.extend(
            ["--nsis-path", f'"{nsis_path}"'])
        ...
        # THE FIX: Execute the batch file directly and set the working directory.
        self._log_to_dialog(f"Executing build script: {build_script} {' '.join(args)}")
        installer_dir = os.path.dirname(build_script)
        self.build_process.setWorkingDirectory(installer_dir)
        self.build_process.start("build.bat", args)
    ...
    def _upload_assets(self):
        ...
        if dialog_data.get("build_installer"):
            ...
            installer_name, installer_path = f"PuffinPyEditor_v{version_str}_Setup.exe", os.path.join(dist_path,
                                                                                                      installer_name)
