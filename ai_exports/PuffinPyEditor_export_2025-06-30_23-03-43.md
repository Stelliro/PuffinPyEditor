...
## Export Timestamp: 2025-06-30T23:03:44.053808
---
...
def show_project_context_menu(panel, position: QPoint, path: str, is_dir: bool):
    ...
    Includes options for copying file paths.
    """
    tree = panel.tree
    ...
    if is_valid_selection:
        ...
        abs_path_action.triggered.connect(lambda: _copy_to_clipboard(path))

        # Relative Path (calculated from the project root)
        try:
            relative_path = os.path.relpath(path, start=panel.project_path)
            rel_path_action = menu.addAction(qta.icon('mdi.link-box-variant-outline'), "Copy Relative Path")
            rel_path_action.triggered.connect(
                lambda: _copy_to_clipboard(relative_path.replace("\\", "/")))  # Normalize to forward slashes
        except ValueError:
            # This can happen on Windows if path is on a different drive than project
            pass
        # --- End of New Actions ---

    menu.addSeparator()
    # Use panel's path if no item is selected (for revealing the root folder)
    reveal_path = path if is_valid_selection else panel.project_path
...
class DraggableTreeView(QTreeView):
    """
    A QTreeView subclass configured for a clean, single-column aesthetic where a
    delegate handles the layout of all text.
    """
    ...
    def __init__(self, puffin_api: PuffinPluginAPI, parent=None):
        ...
        # --- CRITICAL FIX: Disable view-based sorting ---
        # QFileSystemModel is NOT compatible with the internal QSortFilterProxyModel
        # that QTreeView creates when sorting is enabled. This can lead to data
        # corruption in the view (like showing dates instead of filenames) and
        # instability. The model provides its own default alphabetical sorting.
        self.setSortingEnabled(False)
        # --- End of Fix ---
...
class ProjectExplorerPlugin:
    """
    Manages the Project Explorer dock widget. Its primary role is to host a
    QTabWidget, where each tab contains a ProjectExplorerPanel for an open project.
    It synchronizes the open tabs with the application's global ProjectManager.
    """

    def __init__(self, puffin_api: PuffinPluginAPI):
        """
        Initializes the plugin's UI and connects to application-wide signals.
        """
        self.api = puffin_api
        self.main_window = self.api.get_main_window()
        self.project_manager = self.api.get_manager("project")
        ...
        # The main container widget that will be docked.
        self.container = QWidget()
        self.container_layout = QVBoxLayout(self.container)
        self.container_layout.setContentsMargins(0, 0, 0, 0)

        # The tab widget to hold each open project view.
        self.project_tabs = QTabWidget()
        self.project_tabs.setTabsClosable(True)
        self.project_tabs.setMovable(True)
        self.project_tabs.setDocumentMode(True) # A sleek, modern look for tabs.

        self.container_layout.addWidget(self.project_tabs)

        # --- Register with the Main Application ---
        # Register the container as a dock panel on the left side.
        self.api.register_dock_panel(
            self.container,
            "Project Explorer",
            Qt.DockWidgetArea.LeftDockWidgetArea,
            "fa5s.folder-open"  # The icon for the dock panel handle.
        )

        # --- Connect Signals ---
        # Handle closing a project via its tab's 'x' button.
        self.project_tabs.tabCloseRequested.connect(self._handle_tab_close)
        # Handle switching the active project by clicking a different tab.
        self.project_tabs.currentChanged.connect(self._on_project_tab_changed)
        # Ensure the UI appearance stays in sync when the application theme changes.
        self.main_window.theme_changed_signal.connect(self.on_theme_changed)

        # --- Initial Population ---
        # Load any projects that were open from the last session.
        self.refresh_project_views()
        log.info("Project Explorer Plugin initialized.")

    def on_theme_changed(self, theme_id: str):
        """
        Triggered when the global application theme is changed. This method ensures
        that all visible tree views get the new theme's colors, especially for icons.
        """
        log.info("Project Explorer: Theme changed, updating icons.")
        # Iterate through each tab (each is a ProjectExplorerPanel).
        for i in range(self.project_tabs.count()):
            panel = self.project_tabs.widget(i)
            if isinstance(panel, ProjectExplorerPanel):
                # A new IconProvider is created to fetch the new colors from the theme manager.
                panel.model.setIconProvider(CustomFileIconProvider(self.api))
                # Force a complete redraw of the tree view to apply all new styles.
                panel.tree.viewport().update()

    def _handle_tab_close(self, index: int):
        """Closes a project when its corresponding tab is closed by the user."""
        panel = self.project_tabs.widget(index)
        # Call the main window's close project action to ensure consistent behavior.
        if hasattr(panel, 'project_path'):
            self.main_window._action_close_project(panel.project_path)

    def _on_project_tab_changed(self, index: int):
        """Updates the application's active project when the user switches tabs."""
        if index != -1: # -1 means no tab is selected.
            panel = self.project_tabs.widget(index)
            if hasattr(panel, 'project_path'):
                self.project_manager.set_active_project(panel.project_path)
                # Ensure the main window title reflects the newly active project.
                self.main_window._update_window_title()

    def refresh_project_views(self):
        """
        Syncs the project tabs with the master list of open projects from the
        ProjectManager. This is the key method for ensuring the UI is consistent
        with the application's state, handling both newly opened and closed projects.
        """
        open_projects = self.project_manager.get_open_projects()

        # Create a set of project paths for efficient lookup.
        current_tab_paths = {
        ...
        # Step 1: Close tabs for projects that are no longer in the master list.
        # Iterate backwards to safely remove tabs without index-shifting issues.
        ...
        # Step 2: Open new tabs for projects that are in the master list but not in the UI.
        for path in open_projects:
            if path not in current_tab_paths:
                panel = ProjectExplorerPanel(self.api, path)
                self.project_tabs.addTab(panel, os.path.basename(path))

        # Step 3: Ensure the currently selected tab matches the active project.
...
def initialize(puffin_api: PuffinPluginAPI):
    """
    Standard entry point for PuffinPyEditor plugins.
    Called by the PluginManager during application startup.
    """
...
import os
from PyQt6.QtWidgets import QWidget, QVBoxLayout, QMessageBox, QInputDialog
from PyQt6.QtCore import Qt, QDir, QPoint, QThreadPool, QFileSystemWatcher
from PyQt6.QtGui import QFileSystemModel
from app_core.puffin_api import PuffinPluginAPI
from utils.logger import log

from .draggable_tree import DraggableTreeView
from .view_delegate import StyledIndentDelegate, _get_git_status  # Import cache to clear it
from .context_menu import show_project_context_menu
from .icon_provider import CustomFileIconProvider
from .folder_size_worker import FolderSizeWorker


class ProjectExplorerPanel(QWidget):
    """
    The widget for a single project tab, containing the file tree and managing
    background tasks, Git status updates, and live file monitoring.
    """
    ...
    def __init__(self, puffin_api: PuffinPluginAPI, project_path: str, parent=None):
        ...
        self.folder_size_cache: dict[str, int | str] = {}
        self.active_workers: dict[str, FolderSizeWorker] = {}
        self.thread_pool = QThreadPool.globalInstance()
        self.thread_pool.setMaxThreadCount(max(1, QThreadPool.globalInstance().maxThreadCount() // 2))

        # --- Live File System Monitoring for Git Status ---
        self.fs_watcher = QFileSystemWatcher(self)
        self.fs_watcher.directoryChanged.connect(self._on_filesystem_changed)
        self.fs_watcher.fileChanged.connect(self._on_filesystem_changed)
        ...
        # The custom delegate that makes it all look cool
        self.delegate = StyledIndentDelegate(self.tree, self.api, self)
        self.tree.setItemDelegate(self.delegate)

        # Set initial project view and connect signals
        ...
        # Initial calculation for the root
        self.calculate_folder_size(self.project_path)

    def _setup_fs_watcher(self):
        """
        Sets up the QFileSystemWatcher to monitor the project directory for changes.
        This is crucial for providing live Git status updates in the view delegate.
        """
        ...
        if os.path.isdir(self.project_path):
            self.fs_watcher.addPath(self.project_path)
            log.info(f"FileSystemWatcher monitoring: {self.project_path}")

            # Specifically watch key git files for more immediate updates
            git_dir = os.path.join(self.project_path, ".git")
            if os.path.isdir(git_dir):
                self.fs_watcher.addPath(os.path.join(git_dir, "HEAD"))
                self.fs_watcher.addPath(os.path.join(git_dir, "index"))

    def _on_filesystem_changed(self, path: str):
        """
        Slot connected to the file watcher. When any file or directory changes,
        it clears the Git status cache and triggers a repaint of the tree.
        """
        log.debug(f"File system change detected at: {path}. Refreshing view.")

        # This is the magic that makes the Git statuses "live".
        _get_git_status.cache_clear()

        # Redraw the entire tree to reflect any potential new Git statuses.
        self.tree.viewport().update()

        # Some editors (like VSCode) use atomic saves, which can remove the watched path.
        # We re-add it here to ensure we don't stop watching.
        if path not in self.fs_watcher.files() and path not in self.fs_watcher.directories():
            self.fs_watcher.addPath(path)

    # --- Existing Functionality (Mostly Unchanged) ---

    def _on_item_expanded(self, index):
        """When a folder is expanded, trigger size calculations for its children."""
    ...
    def _on_item_collapsed(self, index):
        """When a folder is collapsed, cancel pending calculations for its children."""
        self._cancel_child_workers(index)

    def _cancel_child_workers(self, parent_index):
        for i in range(self.model.rowCount(parent_index)):
            child_index = self.model.index(i, 0, parent_index)
            path = self.model.filePath(child_index)
            if path in self.active_workers:
                self.active_workers[path].cancel()
            if self.model.isDir(child_index) and self.tree.isExpanded(child_index):
                self._cancel_child_workers(child_index)

    def calculate_folder_size(self, path: str):
        """Starts a background worker to calculate a folder's size if not cached."""
        if path in self.folder_size_cache or path in self.active_workers:
            return

        self.folder_size_cache[path] = "..."
        worker = FolderSizeWorker(path)
        worker.signals.finished.connect(self._on_size_calculated)
        self.active_workers[path] = worker
        self.thread_pool.start(worker)

    def _on_size_calculated(self, path: str, size: int):
        """Slot to receive the result from the background worker."""
        if path in self.active_workers:
            del self.active_workers[path]

        self.folder_size_cache[path] = size
        self.tree.viewport().update()  # Redraw to show the new size
    ...
    def _set_project_view(self, project_path):
        ...
        log.info(f"Project Explorer panel root set to: {project_path}")
    ...
    def _action_new_file(self, target_dir: str):
        ...
        if success:
            ...
            QMessageBox.warning(self, "Error Creating File", message)
    ...
    def _action_new_folder(self, target_dir: str):
        ...
        if not success: QMessageBox.warning(self, "Error Creating Folder", message)
    ...
    def _action_delete(self, path: str):
        ...
        reply = QMessageBox.question(self, "Confirm Delete",
                                     ...
                                     QMessageBox.StandardButton.No)
        if reply == QMessageBox.StandardButton.Yes:
            success, message = self.file_handler.delete_item(path)
            if not success: QMessageBox.critical(self, "Delete Error", message)
...
# --- UTILITY FUNCTIONS ---
# Defined at the top level to be importable by other modules like the panel.
...
def _get_git_status(project_root: str) -> dict:
    ...
    try:
        # Use --porcelain v1 for stable, script-friendly output.
        # Add startupinfo for Windows to prevent console windows from popping up.
        startupinfo = None
        if os.name == 'nt':
        ...
        # Gracefully handle cases where git is not installed or not a repo.
        return {}
...
class StyledIndentDelegate(QStyledItemDelegate):
    ...
    def paint(self, painter: QPainter, option: QStyleOptionViewItem, index: QModelIndex):
        ...
        git_status = _get_git_status(self.panel.project_path).get(os.path.normpath(filepath))
        ...
        # Git status color override
        if git_status:
            text_color = QColor(colors.get('git.status.foreground', '#87ceeb'))  # Sky Blue default

        # Selection color overrides everything
        if is_selected:
        ...
        self.paint_indentation_guides(painter, option, index)
        QApplication.style().drawPrimitive(QStyle.PrimitiveElement.PE_IndicatorBranch, style_option, painter, self.tree)

        # --- Layout ---
        content_x = self.tree.visualRect(index).x()
        icon_size = option.decorationSize
        icon_rect = QRect(content_x, option.rect.y() + (option.rect.height() - icon_size.height()) // 2,
        ...
        metadata_rect = QRect(option.rect.right() - metadata_width - 8, option.rect.y(), metadata_width,
                              option.rect.height())
        ...
        git_char_rect = QRect(metadata_rect.left() - git_char_width, option.rect.y(), git_char_width,
                              option.rect.height())

        filename_x = icon_rect.right() + 5
        filename_width = git_char_rect.left() - filename_x - 5
        filename_rect = QRect(filename_x, option.rect.y(), max(0, filename_width), option.rect.height())

        # --- Painting ---
        icon.paint(painter, icon_rect)

        painter.setPen(text_color)
        painter.drawText(filename_rect, int(Qt.AlignmentFlag.AlignVCenter),
                         fm.elidedText(filename, Qt.TextElideMode.ElideRight, filename_rect.width()))
        if git_status_char:
    ...
    def get_metadata_text(self, index: QModelIndex) -> str:
        """Assembles the right-aligned annotation string (size and/or date)."""
        model = index.model()
        date_str, size_str = "", ""
        if (date := model.lastModified(index)).isValid():
            date_str = date.toString("MMM dd hh:mm")

        if model.isDir(index):
            if (size_val := self.panel.folder_size_cache.get(model.filePath(index))) is None:
                self.panel.calculate_folder_size(model.filePath(index))
            elif isinstance(size_val, int) and size_val >= 0:
                size_str = _format_size(size_val)
        else:
            size_str = _format_size(model.size(index))

        return f"{size_str:<10}{date_str}"  # Pad size for nice alignment

    def paint_indentation_guides(self, painter: QPainter, option: QStyleOptionViewItem, index: QModelIndex):
        """Draws VS-Code-style indentation guide lines."""
        theme_mgr = self.api.get_main_window().theme_manager
        guide_color = QColor(
        ...
        while temp_index.parent().isValid():
            ...
            has_next_sibling = parent_index.model().rowCount(parent_index.parent()) > parent_index.row() + 1
            if has_next_sibling:
                x_pos = self.tree.visualRect(parent_index).left() + indent // 2
