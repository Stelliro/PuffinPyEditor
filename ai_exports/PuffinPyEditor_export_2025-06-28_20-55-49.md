...
## Export Timestamp: 2025-06-28T20:55:49.512863
---
...
BUILT_IN_THEMES = {
    ...
    "puffin_light": {
        ...
        "colors": {
            ...
            "tree.indentationGuides.stroke": "#D0D0D0",
            "tree.trace.color": "#007ACC",
            "tree.trace.shadow": "#267f99",
            "tree.node.color": "#007ACC",
            "tree.node.fill": "#E0E0E0"
        }
    }
}
...
# A mock highlighter to prevent crashes until a real one is implemented
...
class EditorWidget(QWidget):
    """
    A basic editor widget that serves as a functional placeholder.
    It allows the main application to run without the full-featured editor.
    """
    ...
    def __init__(self, completion_manager=None, parent=None):
        super().__init__(parent)
        self.filepath = None
        self._completion_manager = completion_manager  # Not used in this placeholder, but kept for API compatibility
        ...
        # A mock highlighter to prevent MainWindow from crashing
    ...
    def set_filepath(self, filepath: str | None):
        """Sets the file path associated with this editor tab."""
        self.filepath = filepath

    def get_text(self) -> str:
        """Returns the entire content of the text area."""
    ...
    def get_cursor_position(self) -> tuple[int, int]:
        """Returns the current (line, column) of the cursor. 1-based."""
    ...
    def update_editor_settings(self):
        """Applies font and tab settings from the settings manager."""
        font_family = settings_manager.get("editor.font.family", "Consolas")
        font_size = settings_manager.get("editor.font.size", 11)
        font = QFont(font_family, font_size)
        self.text_area.setFont(font)

        tab_width = settings_manager.get("editor.tabs.width", 4)
        self.text_area.setTabStopDistance(font.pointSize() * tab_width * 0.7)  # Approximation
```
...
# PyQt6 imports
from PyQt6.QtGui import (QFileSystemModel, QPainter, QPen, QColor, QPolygonF, QPainterPath)
...
class FileTree(QTreeView):
    def __init__(self, model, theme_manager, parent=None):
        ...
        self.setRootIsDecorated(False)  # We are doing all the decoration
        for i in range(1, self.model().columnCount()):
            self.setColumnHidden(i, True)

    def paintEvent(self, event):
        # *** FIX: The custom guide lines are now drawn BEFORE the default items. ***
        ...
        # --- Parameters for the new "schematic" style ---
        bus_width = 1.0
        trace_thickness = 1.2
        node_chip_size = 4.0
        shadow_offset = QPointF(1.0, 1.0)
        kink_size = 5.0  # The length of the 45-degree segment
        ...
        while current_index.isValid():
            ...
            # 2. Draw current item's vertical bus line
            painter.drawLine(QPointF(stem_x, rect.top()), QPointF(stem_x, rect.bottom()))

            # 3. Draw the angular "trace" from the bus to the item
            start_p = QPointF(stem_x, y_connect)
            end_p = QPointF(rect.left() - 6, y_connect)  # 6px margin to text

            # Create an angular path with 45-degree segment
            kink_dir = -1 if current_index.row() % 2 == 0 else 1  # Alternate direction
            p1 = start_p
            p2 = QPointF(p1.x() + kink_size, p1.y())
            p3 = QPointF(p2.x() + kink_size, p2.y() + kink_size * kink_dir)
            p4 = QPointF(end_p.x() - kink_size, p3.y())
            p5 = QPointF(p4.x() + kink_size, end_p.y())
            ...
            # 4. Draw glow/shadow and then the trace itself
            shadow_path = QPainterPath(trace_path)
            shadow_path.translate(shadow_offset)
            painter.setPen(QPen(faded_shadow_color, trace_thickness * 1.5, cap=Qt.PenCapStyle.RoundCap,
        ...
        # *** FIX: Calling super() at the end draws the items (icons, text) on TOP of our custom guides. ***
...
# *** FIX: QActionGroup was already added. Redundant FIX comment removed. ***
# Unused imports related to the file tree have been removed.
...
from app_core.puffin_api import PuffinPluginAPI
# The deferred import is no longer needed; we can import directly now.
from .editor_widget import EditorWidget
from .preferences_dialog import PreferencesDialog
# Import the refactored FileTreeViewWidget
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, file_handler, theme_manager, debug_mode=False, parent=None):
        ...
        self.file_handler = file_handler
        # Set the parent window for the file handler to use in dialogs
    ...
    def _apply_theme_and_icons(self, theme_id: str):
        ...
        for i in range(self.tab_widget.count()):
            ...
            if isinstance(widget, EditorWidget):
                ...
                widget.update_theme()
        # Also update project tree views
        for i in range(self.project_tabs.count()):
            if (tree_widget := self.project_tabs.widget(i)) and isinstance(tree_widget, FileTreeViewWidget):
                tree_widget.update_theme()
    ...
    def _initialize_project_views(self):
        ...
        # Preserve existing tree widgets if possible to maintain state
        current_trees = {self.project_tabs.tabToolTip(i): self.project_tabs.widget(i) for i in
                         range(self.project_tabs.count())}
    ...
    def _is_editor_modified(self, editor: 'QWidget') -> bool:
        ...
        if not data:
            return False
        current_content = editor.get_text()
        return hash(current_content) != data['original_hash']
    ...
    def _on_content_changed(self):
        ...
        if index != -1:
            ...
            # Add or remove asterisk based on modified state
        ...
        if self.settings.get("auto_save_on_change", False):
            delay = self.settings.get("auto_save_delay_ms", 1500)
    ...
    def _on_project_tab_changed(self, index):
        if index != -1:
            ...
            self.project_manager.set_active_project(None)
        self._update_window_title()
    ...
    def _update_window_title(self):
        ...
        if isinstance(current_widget, EditorWidget):
            editor_data = self.editor_tabs_data.get(current_widget)
            filepath = editor_data.get('filepath') if editor_data else None

            if filepath:
                current_file = os.path.basename(filepath)
            else:
                current_file = self.tab_widget.tabText(self.tab_widget.currentIndex()).replace(" *", "")

            if self._is_editor_modified(current_widget):
                current_file += " *"

        parts = [current_file, project_name, base_title]
    ...
    def _action_save_file(self, editor_widget=None, save_as=False):
        ...
        if not isinstance(current_editor, EditorWidget):
            return None

        editor_data = self.editor_tabs_data.get(current_editor)
        if not editor_data:
            log.error("Could not find data for the current editor tab.")
            return None

        content = current_editor.get_text()
        is_modified = self._is_editor_modified(current_editor)
        ...
        filepath = self.file_handler.save_file_content(
            editor_data['filepath'], content, save_as=save_as
        )
        ...
        if filepath:
            ...
            self._update_window_title()
            self._on_content_changed()  # To remove asterisk
            return filepath
        else:
    ...
    def _action_save_all(self):
        ...
        for i in range(self.tab_widget.count()):
            ...
            if self._is_editor_modified(widget):
                if self._action_save_file(editor_widget=widget):
                    saved_count += 1
        if saved_count > 0:
    ...
    def _action_close_tab_by_index(self, index):
        ...
        if self._is_editor_modified(widget_to_close):
            ...
            msg_box.setText(f"Do you want to save the changes to {filename}?")
            ...
            if ret == QMessageBox.StandardButton.Save:
                if not self._action_save_file(editor_widget=widget_to_close):
                    return  # User cancelled save dialog
            elif ret == QMessageBox.StandardButton.Cancel:
                return  # Abort close

        self.tab_widget.removeTab(index)
        if widget_to_close in self.editor_tabs_data:
            del self.editor_tabs_data[widget_to_close]
    ...
    def _show_about_dialog(self):
        QMessageBox.about(self, "About PuffinPyEditor",
                          f"<b>PuffinPyEditor v{versioning.APP_VERSION}</b>"
                          "<p>A modern, extensible code editor written in Python.</p>"
                          "<p>Copyright © 2024 PuffinPyEditorProject. "
                          "All rights reserved.</p>")

    def _open_github_link(self):
        QDesktopServices.openUrl(QUrl("https://github.com/your-repo/PuffinPyEditor"))

    def _auto_save_current_tab(self):
        current_editor = self.tab_widget.currentWidget()
        if self._is_editor_modified(current_editor):
            log.info(f"Auto-saving tab: {self.tab_widget.tabText(self.tab_widget.currentIndex())}")
            self._action_save_file(editor_widget=current_editor)
    ...
    def closeEvent(self, event: QCloseEvent):
        ...
        for i in range(self.tab_widget.count() - 1, -1, -1):
            ...
            if self._is_editor_modified(widget):
                self.tab_widget.setCurrentIndex(i)
                filename = self.tab_widget.tabText(i).replace(" *", "")
                msg_box = QMessageBox(self)
                msg_box.setIcon(QMessageBox.Icon.Question)
                msg_box.setText(f"Do you want to save the changes to {filename}?")
                msg_box.setStandardButtons(
                ...
                if ret == QMessageBox.StandardButton.Save:
                    if not self._action_save_file(editor_widget=widget):
...
def main():
    ...
    # The handler is created without a parent window, which will be set inside MainWindow
    file_handler = FileHandler()

    # Import MainWindow inside main() to prevent circular dependency
    # Assuming ui.editor_widget.py now exists and is correct.
