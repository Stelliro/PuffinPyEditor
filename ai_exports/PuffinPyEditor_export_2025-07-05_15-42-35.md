...
## Export Timestamp: 2025-07-05T15:42:35.608263
---
...
class PuffinPluginAPI:
    ...
    def add_dock_panel(self, widget: QWidget, title: str, area_str: str, icon_name: Optional[str] = None):
        ...
        # It was missing before, so plugins couldn't add panels.
...
# MODIFICATION: The highlighter is no longer imported or used here.
...
class CppToolsPlugin(QObject):
    def __init__(self, puffin_api: PuffinPluginAPI):
        ...
        if not self.output_panel:
            log.error("C++ Tools: Could not find Output Panel. This plugin needs python_tools.")
            return
        self._setup_ui()
        self._connect_signals()

    def _get_output_panel(self):
        # MODIFICATION: Corrected typo 'pythong_tools' to 'python_tools'
        ...
        if index != -1:
            ...
            if isinstance(widget, EditorWidget):
                ...
                if data and (fp := data.get('filepath', '')):
                    is_cpp = fp.lower().endswith(('.cpp', '.c', '.cxx', '.h', '.hpp'))
...
from app_core.puffin_api import PuffinPluginAPI
from utils.logger import log
# DELETED: The import for the highlighter is gone. It's no longer needed here.
...
class CSharpToolsPlugin(QObject):
    def __init__(self, puffin_api: PuffinPluginAPI):
        ...
        self.output_panel = self._get_output_panel()
        if not self.output_panel:
            log.error("C# Tools: Could not find Output Panel. This plugin depends on Python Tools.")
            return

        # DELETED: The call to _setup_highlighter is gone.
        self._setup_ui()
        self._connect_signals()
    ...
    # DELETED: The entire _setup_highlighter method is gone.
...
from app_core.puffin_api import PuffinPluginAPI
from utils.logger import log
# DELETED: The import for the highlighter is gone.
...
```python
# PuffinPyEditor/plugins/python_tools/plugin_main.py
import os
from PyQt6.QtWidgets import QMessageBox
...
class PythonToolsPlugin(QObject):
    def __init__(self, puffin_api: PuffinPluginAPI):
        super().__init__()
        self.api = puffin_api
        self.main_window = self.api.get_main_window()
    ...
    def _setup_highlighter(self):
        ...
        self.api.log_info("Python Tools: Registered highlighter for .py files.")
...
def initialize(puffin_api: PuffinPluginAPI):
    return PythonToolsPlugin(puffin_api)
```
...
class TerminalPlugin:
    ...
    def _setup_ui(self):
        ...
        # MODIFIED: Use the corrected method name from puffin_api.py
...
class StyledTreeView(QTreeWidget):
    ...
    def dragMoveEvent(self, event: 'QDragMoveEvent'):
        ...
        source_path = source_bytes.data().decode('utf-8')
...
class FileSystemListView(QWidget):
    ...
    def _setup_ui(self):
        ...
        self.expand_button.setIcon(qta.icon('fa5s.expand-arrows-alt', color='gray'))
        self.expand_button.setToolTip("Expand All")

        self.collapse_button = QToolButton()
        self.collapse_button.setIcon(qta.icon('fa5s.compress-arrows-alt', color='gray'))
        self.collapse_button.setToolTip("Collapse All")

        self.refresh_button = QToolButton()
        self.refresh_button.setIcon(qta.icon('fa5s.sync-alt', color='gray'))
    ...
    def get_expanded_paths(self):
        expanded_set = set()
        self._get_expanded_children_recursive(self.tree_widget.invisibleRootItem(), expanded_set)
        return list(expanded_set)

    def _get_expanded_children_recursive(self, item, expanded_set):
        if item and item.isExpanded():
            data = item.data(0, Qt.ItemDataRole.UserRole)
            if data and (path := data.get('path')):
                expanded_set.add(os.path.normpath(path))
            for i in range(item.childCount()):
                self._get_expanded_children_recursive(item.child(i), expanded_set)
    ...
    def _add_item_to_tree(self, parent_item, path):
        ...
        item.setData(0, Qt.ItemDataRole.UserRole, {'path': path, 'is_dir': is_dir})

        if is_dir:
            item.addChild(QTreeWidgetItem())  # Placeholder for expander arrow
    ...
    def _on_item_created(self, item_type: str, path: str):
        log.debug(f"Handling item creation for: {path}")
        parent_dir = os.path.dirname(path)
        parent_item = self._find_item_by_path(self.tree_widget.invisibleRootItem(), parent_dir)
        if not parent_item:
            log.error(f"Cannot add tree item: parent '{parent_dir}' not found in tree.")
            return

        is_populated = not (parent_item.childCount() == 1 and not parent_item.child(0).text(0))
        if not parent_item.isExpanded() or not is_populated:
            parent_item.setExpanded(True)
        else:
            self._add_item_to_tree(parent_item, path)
    ...
    def _on_item_deleted(self, item_type: str, path: str):
        item = self._find_item_by_path(self.tree_widget.invisibleRootItem(), path)
    ...
    def _schedule_refresh(self, path: str):
        if not self._internal_change_flag:
    ...
    def refresh(self):
        ...
        current_path = None
        if current_item and (data := current_item.data(0, Qt.ItemDataRole.UserRole)):
        ...
        for path in open_projects:
            self.git_statuses.update(get_git_statuses_for_root(path))
            self._add_to_watcher(path)
        ...
        for path in open_projects:
            item = QTreeWidgetItem(self.tree_widget, [os.path.basename(path)])
            item.setToolTip(0, path)
            colors = self.theme_manager.current_theme_data.get('colors', {})
            item.setIcon(0, qta.icon('fa5s.folder-open', color=colors.get('accent')))
            item.setData(0, Qt.ItemDataRole.UserRole, {'path': path, 'is_dir': True, 'is_root': True})
    ...
    def _find_item_by_path(self, parent, path_to_find):
        ...
        for i in range(parent.childCount()):
            ...
            data = child.data(0, Qt.ItemDataRole.UserRole)
            if data and os.path.normpath(data.get('path', '')) == norm_path:
                return child

            if child.isExpanded():
                found_item = self._find_item_by_path(child, path_to_find)
                if found_item:
                    return found_item
        return None

    def _populate_node(self, parent_item, path, preserve_expanded=False):
        if not os.path.isdir(path):
            return

        expanded_children_paths = set()
        if preserve_expanded:
            for i in range(parent_item.childCount()):
                child = parent_item.child(i)
                if child and child.isExpanded() and (data := child.data(0, Qt.ItemDataRole.UserRole)):
                    expanded_children_paths.add(data.get('path'))
        ...
        try:
            ...
            if project_root:
                self.git_statuses = get_git_statuses_for_root(project_root)
                get_git_statuses_for_root.cache_clear()
            ...
            for entry in entries:
                if entry.name.startswith(('__', '.')) and entry.name != '.gitignore':
                    continue
                ...
                if status:
                    if status == '??':
                        item.setForeground(0, untracked_color)
                    elif 'M' in status:
                        item.setForeground(0, modified_color)
                    elif status == '!!':
                        item.setForeground(0, ignored_color)

                if entry.is_dir():
                    item.addChild(QTreeWidgetItem())
                    self._add_to_watcher(entry.path)

                if preserve_expanded and entry.path in expanded_children_paths:
    ...
    def on_item_expanded(self, item: QTreeWidgetItem):
        ...
        # Populate if it's a directory with a placeholder child
        if path and item.childCount() == 1 and not item.child(0).text(0):
            self._populate_node(item, path, preserve_expanded=True)

    def on_item_collapsed(self, item: QTreeWidgetItem):
        data = item.data(0, Qt.ItemDataRole.UserRole)
        # Don't collapse root project nodes
        if data and not data.get('is_root', False):
            item.takeChildren()
            item.addChild(QTreeWidgetItem())  # Add placeholder back
    ...
    def show_context_menu(self, position: QPoint):
        ...
        if item and (data := item.data(0, Qt.ItemDataRole.UserRole)):
            ...
            path = self.project_manager.get_active_project_path()
...
from PyQt6.QtWidgets import (QWidget, QPlainTextEdit, QTextEdit, QHBoxLayout, QVBoxLayout)
...
class PythonMiniMap(MiniMapWidget):
    """A specialized minimap that visualizes the structure of Python code."""
    CLASS_RE = re.compile(r"^\s*class\s+(\w+)")
    DEF_RE = re.compile(r"^\s*def\s+(_?[a-zA-Z0-9_]+)")
    ...
    def _precompute_structure(self) -> List[Tuple[int, int, str, str]]:
        ...
        for i in range(doc.blockCount()):
            ...
            class_match = self.CLASS_RE.match(text)
            def_match = self.DEF_RE.match(text)
            indent = self._get_indent(text)

            if class_match:
                structure.append((i, indent, "class", class_match.group(1)))
            elif def_match:
                structure.append((i, indent, "def", def_match.group(1)))
        return structure

    def _format_name(self, name: str) -> str:
        """Strips underscores and converts snake_case to Title Case for readability."""
    ...
    def paintEvent(self, event: QPaintEvent):
        ...
        class_bg = QColor(class_color)
        class_bg.setAlpha(30)
        func_bg = QColor(func_color)
        func_bg.setAlpha(30)
        ...
        for i, (start_line, indent, type, name) in enumerate(structure):
            end_line = doc.blockCount() - 1
            for j in range(i + 1, len(structure)):
                if structure[j][1] <= indent:
                    end_line = structure[j][0] - 1
                    break

            block_y_start = scroll_offset + (start_line * line_height_map * scale)
            block_y_end = scroll_offset + ((end_line + 1) * line_height_map * scale)
            block_height = block_y_end - block_y_start

            bg_color = class_bg if type == "class" else func_bg
            border_color = class_color if type == "class" else func_color

            block_indent = indent * 1.5
            block_rect = QRectF(block_indent, block_y_start, self.width() - block_indent - 1, block_height)
            painter.fillRect(block_rect, bg_color)
            painter.setPen(QPen(border_color, 1))
            painter.drawRect(block_rect)

            label_y = max(block_y_start, y_cursor)
            ...
            label_rect = QRectF(text_indent, label_y, self.width() - text_indent - 5, 12)
            # THE FIX: Combine flags correctly using the | operator.
            painter.drawText(label_rect, Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.TextSingleLine, formatted_name)
...
class EditorWidget(QWidget):
    ...
    def _create_minimap(self) -> MiniMapWidget:
        if self.filepath and self.filepath.endswith('.py'):
            log.info("Creating Python-specific minimap.")
            return PythonMiniMap(self.text_area)
        else:
    ...
    def _setup_layout(self):
        ...
        editor_layout.addWidget(self.minimap_widget)
    ...
    def set_filepath(self, filepath: Optional[str]):
        ...
        if new_minimap.__class__ != self.minimap_widget.__class__:
            parent_layout = self.minimap_widget.parentWidget().layout()
            if parent_layout:
                parent_layout.replaceWidget(self.minimap_widget, new_minimap)
...
from app_core.highlighters.html_syntax_highlighter import HtmlSyntaxHighlighter
from app_core.highlighters.cpp_syntax_highlighter import CppSyntaxHighlighter
from app_core.highlighters.csharp_syntax_highlighter import CSharpSyntaxHighlighter
from app_core.highlighters.javascript_syntax_highlighter import JavaScriptSyntaxHighlighter
from app_core.highlighters.rust_syntax_highlighter import RustSyntaxHighlighter
