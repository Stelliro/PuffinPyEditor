...
## Export Timestamp: 2025-07-09T16:56:48.102847
---
...
from scene.entities import BaseEntity
...
class PropertiesPanel(UIPanel):
    def __init__(self, app_state: AppState, event_bus: EventBus, **kwargs):
        ...
        event_bus.sub(Event.SELECTION_CHANGED, self.rebuild)
        event_bus.sub(Event.ENTITY_UPDATED, self.rebuild)
        self.rebuild()

    def rebuild(self, entity: Optional[BaseEntity] = None):
        ...
        if data:
            ...
            if data.entity_type != 'Room':
                delete_btn = Button(parent=self.content_parent, text='Delete', scale_y=0.04, y=y_cursor,
                                    color=config.THEME['delete_button'])
                delete_btn.text_entity.color = config.THEME['text']
                delete_btn.on_click = lambda: self.event_bus.pub(Event.DELETE_SELECTED)
        else:
        ...
        self.filename_input = InputField(parent=self.content_parent, default_value=self.app_state.project_name,
                                         y=y_cursor, scale_y=0.04)
        self.filename_input.text_field.text_entity.color = config.THEME['text']

        y_cursor -= 0.05
        save = Button(parent=self.content_parent, text="Save", scale_x=0.4, scale_y=0.04, x=-0.22, y=y_cursor, color=config.THEME['button'])
        save.text_entity.color = config.THEME['text']
        
        load = Button(parent=self.content_parent, text="Load", scale_x=0.4, scale_y=0.04, x=0.22, y=y_cursor, color=config.THEME['button'])
        load.text_entity.color = config.THEME['text']
        
        save.on_click = lambda: self.event_bus.pub(Event.SAVE_PROJECT, self.filename_input.text)
        load.on_click = lambda: self.event_bus.pub(Event.LOAD_PROJECT, self.filename_input.text)

    def _create_vec3_inputs(self, name, attr, data, y):
        Text(parent=self.content_parent, text=name, y=y, x=-0.45, scale=1.1, color=config.THEME['text'])

        def make_submit(i):
            def submit():
                try:
                    new_vec = list(getattr(self.app_state.entities[data.uid], attr)); new_vec[i] = float(inputs[i].text)
                    self.event_bus.pub(Event.ENTITY_UPDATED, uid=data.uid, **{attr: new_vec})
        ...
        for i, val in enumerate(getattr(data, attr)):
            ...
            field.x = -0.15 + i * 0.22
            field.text_field.text_entity.color = config.THEME['text']
            field.on_submit = make_submit(i);
            inputs.append(field)

    def _create_slider(self, text, key, y):
        Text(parent=self.content_parent, text=text, y=y, x=-0.45, scale=1.1, color=config.THEME['text'])
        slider = Slider(parent=self.content_parent, min=0, max=100, default=self.app_state.audio.get(key, 0), y=y, x=0.15,
                        scale_x=0.5, knob_color=config.THEME['slider_knob'])
    ...
    def _create_toggle(self, text, group, key, y):
        Text(parent=self.content_parent, text=text, y=y, x=-0.45, scale=1.1, color=config.THEME['text'])
        target_obj = getattr(self.app_state, group)
        button = Button(parent=self.content_parent, text='', y=y, x=0.15, scale_x=0.5, scale_y=0.04)

        def update():
            is_on = bool(target_obj.get(key) if isinstance(target_obj, dict) else getattr(target_obj, key, False))
            button.text = 'On' if is_on else 'Off'
            button.color = config.THEME['button_active'] if is_on else config.THEME['button']
            button.text_entity.color = config.THEME['text']

        def on_click():
            current_value = bool(target_obj.get(key) if isinstance(target_obj, dict) else getattr(target_obj, key, False))
...
class DraggableWindow(Draggable):
    def __init__(self, title: str, **kwargs):
        ...
        super().__init__(
            parent=camera.ui, # Ensure it's on the UI layer
            scale=window_scale,
            origin=(0, 0),
            lock=(0, 0, 1),
            color=color.clear,
        ...
        self.background = Entity(parent=self, model='quad', scale=1, color=config.THEME['panel_bg'], z=0.1)
        ...
        self.title_bar = Button(
            ...
            model='quad',
            text=title,
            scale=(1, 0.08 / self.scale_y), # Adjust for parent scale
            y=0.5 - (0.08 / self.scale_y / 2),
            color=config.THEME['header_bg'],
            highlight_color=config.THEME['header_bg'],  # No color change on hover
        )
        self.title_bar.text_entity.color = config.THEME['text']
        self.title_bar.text_entity.scale = (0.8, 0.8) # Prevent large text

        # Make the window draggable only by its title bar
        self.drag_handle = self.title_bar
        # The content area should not be draggable
        self.background.collider = 'box'
        self.background.on_click = Func(self.drag_handle.on_click)
...
file_to_clean = 'app.py'
backup_file = 'app.py.bak'
content = None

print(f"--- Cleaning {file_to_clean} ---")

# 1. Read the potentially corrupted file
try:
    with open(file_to_clean, 'r', encoding='utf-8-sig') as f:
        content = f.read()
    print(f"Successfully read {file_to_clean}")
except Exception as e:
...
try:
    ...
    os.rename(file_to_clean, backup_file)
    print(f"Original file backed up to {backup_file}")
except Exception as e:
    print(f"ERROR: Could not back up file: {e}")
    exit(1)

# 3. Write it back cleanly (standard UTF-8)
try:
    with open(file_to_clean, 'w', encoding='utf-8') as f:
        f.write(content)
    print(f"Successfully wrote clean version of {file_to_clean}")
except Exception as e:
    print(f"ERROR: Could not write clean file: {e}")
    os.rename(backup_file, file_to_clean) # Restore backup
    exit(1)

print("\nCleaning complete. Please run 'python main.py' again.")
```
...
```text
An error occurred: function() argument 'code' must be code, not str

Traceback (most recent call last):
  File "C:\Users\gike5\Desktop\AI Python\soundscape_designer\main.py", line 7, in <module>
...
if __name__ == '__main__':
    try:
        # Use runpy to execute app.py, which allows it to be treated as the main script
        runpy.run_path('app.py', run_name='__main__')
    except Exception as e:
        # If any exception occurs, log it to crash_log.txt and also print to console
