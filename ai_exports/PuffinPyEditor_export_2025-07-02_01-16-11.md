...
## Export Timestamp: 2025-07-02T01:16:11.363517
---
...
class FindReplacePlugin:
    ...
    def __init__(self, puffin_api):
        ...
        # Create the main menu action for Find/Replace
        ...
        # Create the separate toolbar action
    ...
    def shutdown(self):
        """
        Cleans up all resources created by the plugin before it is unloaded.
        This is crucial for a clean reload process.
        """
        log.info("Shutting down Find/Replace plugin...")

        # Disconnect signals to prevent calls to methods on a deleted object
        try:
            self.main_window.tab_widget.currentChanged.disconnect(self._on_tab_changed)
        except TypeError:
            log.warning("Could not disconnect tab_changed signal for Find/Replace.")
            pass

        # Remove actions from the UI
        # This part for menus is correct.
        if self.find_action and self.find_action.menu():
            self.find_action.menu().removeAction(self.find_action)

        # THE FIX - Part 2: Use the stored toolbar reference to remove the action.
        if self.main_toolbar and self.toolbar_find_action:
            self.main_toolbar.removeAction(self.toolbar_find_action)

        # Schedule the UI elements for deletion
        if self.find_action:
            self.find_action.deleteLater()
        if self.toolbar_find_action:
            self.toolbar_find_action.deleteLater()
        if self.find_panel:
            self.find_panel.deleteLater()
    ...
    def toggle_find_panel(self):
        """Shows or hides the find panel within the currently active editor."""
        editor = self.main_window.tab_widget.currentWidget()
        ...
        if self.find_panel.isVisible():
            ...
            # Attach the panel to the editor's vertical layout.
            self.find_panel.setParent(editor.editor_area_widget)
            editor.vertical_editor_layout.insertWidget(0, self.find_panel)
    ...
    def _on_tab_changed(self, index: int):
        """Enables the find action only if the current tab is a real code editor."""
        widget = self.main_window.tab_widget.widget(index)
        ...
        # Hide the find panel if we switch to a non-editor tab
...
class BreakpointArea(QWidget):
    ...
    # [MODIFIED] Correctly detect which line is being hovered over
    def mouseMoveEvent(self, event: QMouseEvent):
        text_area = self.editor_widget.text_area

        # Use the y-coordinate of the event and a fixed x-coordinate (e.g., 1)
        # inside the text area's viewport. This correctly finds the text block
        # at the corresponding vertical position, avoiding a coordinate mismatch crash.
        y_pos = event.pos().y()
        cursor = text_area.cursorForPosition(QPoint(1, y_pos))

        if cursor.isNull() or not cursor.block().isValid():
            line_num = -1
        else:
        ...
        if line_num != self.hovered_line:
            ...
            self.update()  # Trigger a repaint to show the hover indicator
    ...
    def paintEvent(self, event) -> None:
        ...
        # MODIFIED: Safely get colors dictionary to prevent crash
...
# Note: The following imports assume these files exist and are correct.
...
class CodeTextEdit(QPlainTextEdit):
    """
    A QPlainTextEdit subclass that handles the painting of its associated
    line number area, ensuring perfect alignment and theme consistency.
    """

    def __init__(self, parent_widget):
        super().__init__(parent_widget)
        self.editor_widget = parent_widget
        self.line_number_area = LineNumberArea(self)
        self.update_line_number_area_width()

    def line_number_area_paint_event(self, event: QRect):
        """Handles the painting of the line number area."""
        painter = QPainter(self.line_number_area)
        theme_data = self.editor_widget.theme_manager.current_theme_data or {}
        colors = theme_data.get('colors', {})

        bg_color = QColor(colors.get('editorGutter.background', '#2c313a'))
        painter.fillRect(event, bg_color)
        ...
        while block.isValid() and top <= event.bottom():
            if block.isVisible() and bottom >= event.top():
                number = str(block_number + 1)
                ...
                painter.drawText(0, int(top), self.line_number_area.width() - 8, self.fontMetrics().height(),
    ...
    def resizeEvent(self, event):
        ...
        cr = self.contentsRect()
        self.line_number_area.setGeometry(
            QRect(cr.left(), cr.top(), self.editor_widget.line_number_area_width(), cr.height()))

    def update_line_number_area_width(self):
        """Sets the viewport margin to make space for the line number area."""
        self.setViewportMargins(self.editor_widget.line_number_area_width(), 0, 0, 0)

    def update_line_number_area(self, rect, dy):
        """Updates the line number area on scroll or content changes."""
        if dy:
            self.line_number_area.scroll(0, dy)
        else:
...
class LineNumberArea(QWidget):
    """
    A QWidget that acts as a painting surface for the CodeTextEdit's line numbers.
    It relies on its parent (CodeTextEdit) to handle the painting logic.
    """
    ...
    def sizeHint(self):
        return QSize(self.code_editor.editor_widget.line_number_area_width(), 0)

    def paintEvent(self, event):
        self.code_editor.line_number_area_paint_event(event.rect())
...
class EditorWidget(QWidget):
    ...
    def __init__(self, completion_manager=None, parent=None):
        ...
        # FIX: Re-establish a stable layout hierarchy. This is the definitive fix for the crash.
        ...
        # This container is crucial for stability and for plugins like Find/Replace.
        self.editor_area_widget = QWidget()
        self.vertical_editor_layout = QVBoxLayout(self.editor_area_widget)
        self.vertical_editor_layout.setContentsMargins(0, 0, 0, 0)
        self.vertical_editor_layout.setSpacing(0)
        self.vertical_editor_layout.addWidget(self.text_area)

        # Add the breakpoint area and the editor container to the main horizontal layout.
        self.main_layout.addWidget(self.breakpoint_area)
        self.main_layout.addWidget(self.editor_area_widget, 1)
        # End of layout fix.

        # --- Signal Connections ---
        self.text_area.blockCountChanged.connect(self.text_area.update_line_number_area_width)
        self.text_area.updateRequest.connect(self.text_area.update_line_number_area)
    ...
    def set_highlighter(self, highlighter_class):
        """Sets a syntax highlighter for the text area."""
    ...
    def _toggle_breakpoint(self, line_num):
        """Adds or removes a breakpoint and updates the display."""
    ...
    def _on_cursor_pos_changed(self):
        """Handles cursor position changes to update UI elements."""
    ...
    def _on_editor_update_request(self, rect, dy):
        """Handles repaint requests from the text editor, e.g., on scroll."""
        self.highlight_current_line()

    def highlight_current_line(self):
        """Applies a background color to the line with the cursor."""
        selections = []
        if not self.text_area.isReadOnly():
            selection = QTextEdit.ExtraSelection()
            theme_data = self.theme_manager.current_theme_data or {}
            colors = theme_data.get('colors', {})
    ...
    def set_text(self, text: str):
        ...
        # Setting text might not move the cursor, so we manually update the highlight
        self._on_cursor_pos_changed()

    def set_filepath(self, filepath: str | None):
        self.filepath = filepath
        is_python_file = filepath and filepath.lower().endswith('.py')
        self.breakpoint_area.setVisible(is_python_file)
    ...
    def replace_current(self, query: str, replace_text: str, flags: QTextDocument.FindFlag) -> bool:
        ...
        # Case-insensitive comparison if the flag is not set
        is_match = (selected_text.lower() == query.lower() if
    ...
    def replace_all(self, query: str, replace_text: str, flags: QTextDocument.FindFlag) -> int:
        ...
        while self.text_area.find(query, flags):
            # insertText operates on the current cursor, which is moved by find()
            self.text_area.textCursor().insertText(replace_text)
            count += 1
        return count

    def update_editor_settings(self):
        """Applies font, indent, and visibility settings from the settings manager."""
        ...
        self.text_area.line_number_area.setVisible(settings_manager.get("show_line_numbers", True))
        self.update_theme()

    def update_theme(self):
        """Applies color theme to the editor and its components."""
        colors = self.theme_manager.current_theme_data.get("colors", {})
        stylesheet = f"""
        QPlainTextEdit {{
