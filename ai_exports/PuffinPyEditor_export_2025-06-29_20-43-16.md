...
## Export Timestamp: 2025-06-29T20:43:16.455491
---
...
```python
#
# PuffinPyEditor - Project Explorer Plugin Panel (UPGRADED VERSION)
# Version: 9.0 "The Fusion"
# This file is based on your original code, now with all features integrated.
#
...
# --- DEFINITIVE QT IMPORTS ---
# Upgraded to correctly handle PyQt6 and its changes, which you have installed.
try:
    # --- PyQt6 Imports ---
    ...
    # QFileSystemModel was moved from QtWidgets to QtGui in Qt6
    from PyQt6.QtGui import QFileSystemModel, QIcon, QDropEvent
    ...
    try:
        ...
        from PySide6.QtWidgets import (QWidget, QVBoxLayout, QTreeView,
                                       QMenu, QMessageBox, QInputDialog, QHeaderView,
                                       QApplication, QMainWindow, QAbstractItemView)
        from PySide6.QtGui import QFileSystemModel, QIcon, QDropEvent
        from PySide6.QtCore import Qt, QDir, QModelIndex
    except ImportError:
        # --- Legacy PyQt5 Fallback ---
        print("Warning: Could not find PyQt6 or PySide6, falling back to legacy PyQt5.")
...
# --- NEW FEATURE: The Draggable Tree View Class ---
class DraggableTreeView(QTreeView):
    """
    A QTreeView subclass with enhanced drag-and-drop logic for moving
    ...
    def dropEvent(self, event: QDropEvent):
        """ Handles the dropping of a file or folder. """
        source_urls = event.mimeData().urls()
        if not source_urls:
            event.ignore();
            return
        ...
        destination_path = model.rootPath()
        if destination_index.isValid():
            path = model.filePath(destination_index)
            destination_path = os.path.dirname(path) if not model.isDir(destination_index) else path

        for url in source_urls:
            source_path = url.toLocalFile()
            if os.path.isdir(source_path):
                if os.path.normpath(destination_path).startswith(os.path.normpath(source_path)):
                    QMessageBox.warning(self, "Invalid Move", "Cannot move a folder into its own subdirectory.")
                    continue
            try:
                ...
                QMessageBox.critical(self, "Move Error", f"Could not move item.\n\nError: {e}")
                event.ignore();
                return


# --- The Original Panel Class, Now UPGRADED ---
class ProjectExplorerPanel(QWidget):
    """
    A widget that displays the project file system and allows interaction.
    """
    ...
    def __init__(self, parent=None):
        ...
        # --- UPGRADE: Use our new DraggableTreeView instead of a standard QTreeView ---
        self.tree = DraggableTreeView(self)
        self.tree.setModel(self.model)
        self.tree.setContextMenuPolicy(Qt.ContextMenuPolicy.CustomContextMenu)
        self.tree.customContextMenuRequested.connect(self._show_context_menu)

        # --- UI Customization (from original code) ---
        self.tree.header().setSectionResizeMode(0, QHeaderView.ResizeMode.Stretch)
        for i in range(1, self.model.columnCount()):
            self.tree.hideColumn(i)  # Hides Size, Type, Date Modified, etc.

        self.layout.addWidget(self.tree)
        self.setLayout(self.layout)
        # Your original code used this default, which is good for general use.
        self.open_project(os.path.expanduser("~"))

    def open_project(self, project_path):
        """Sets the root directory for the project explorer."""
        if not os.path.isdir(project_path):
            print(f"Error: Project path '{project_path}' does not exist.");
            return

        self.model.setRootPath(project_path)
        self.tree.setRootIndex(self.model.index(project_path))
        print(f"Project explorer opened at: {project_path}")
    ...
    def _show_context_menu(self, position):
        ...
        # --- FIX: Using the corrected and reliable icon name ---
        rename_icon = qta.icon('mdi.pencil-outline')
        delete_icon = qta.icon('mdi.delete-outline', color='crimson')

        menu.addAction(rename_icon, "Rename...", partial(self._action_rename, path))
        menu.addAction(delete_icon, "Delete", partial(self._action_delete, path))
    ...
    def _action_delete(self, path):
        """Handles the deletion of a file or folder with confirmation."""
...
# --- Standalone Test Runner (Kept from original code for easy testing) ---
if __name__ == '__main__':
    app = QApplication(sys.argv)
    app.setApplicationName("PuffinPyEditor-Test")
    main_window = QMainWindow()
    main_window.setWindowTitle("Project Explorer Test")
    main_window.setGeometry(100, 100, 350, 600)
    project_explorer = ProjectExplorerPanel()
    test_dir = os.path.join(os.getcwd(), "test_project_standalone")
    os.makedirs(os.path.join(test_dir, "src", "components"), exist_ok=True)
    with open(os.path.join(test_dir, "main.py"), "w") as f: f.write("# Test file\n")
    with open(os.path.join(test_dir, "src", "utils.py"), "w") as f: f.write("# Utils file\n")
    project_explorer.open_project(test_dir)
    main_window.setCentralWidget(project_explorer)
    main_window.show()
    sys.exit(app.exec())
```
