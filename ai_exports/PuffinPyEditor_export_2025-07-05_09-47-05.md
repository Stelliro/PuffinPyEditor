...
## Export Timestamp: 2025-07-05T09:47:05.540430
---
...
### File: `/plugins/html_tools/__init__.py`

```python

```

### File: `/plugins/html_tools/html_syntax_highlighter.py`
...
### File: `/plugins/html_tools/plugin.json`
...
```json
{
    ...
    "author": "PuffinPy Team",
    "version": "1.0.0",
    "description": "Adds syntax highlighting and a 'View in Browser' action for HTML files.",
    "entry_point": "plugin_main.py",
    "dependencies": {}
}
```

### File: `/plugins/html_tools/plugin_main.py`
...
class HtmlToolsPlugin(QObject):
    ...
    def _connect_signals(self):
        self.main_window.tab_widget.currentChanged.connect(self._on_tab_changed)
        # MODIFIED: Defer the initial check to prevent import cycle during startup
        QTimer.singleShot(0, lambda: self._on_tab_changed(self.main_window.tab_widget.currentIndex()))
    ...
    def _on_tab_changed(self, index):
        ...
        if index != -1:
            ...
            if isinstance(widget, EditorWidget):
                data = self.main_window.editor_tabs_data.get(widget)
                if data and (filepath := data.get('filepath', '')):
                    is_html_file = filepath.lower().endswith(('.html', '.htm'))
        ...
        if self.output_panel is None:
            ...
            if not runner_plugin:
                self.api.show_message("critical", "Dependency Missing",
                                      "The Pythong Tools plugin, which provides the Output panel, is not loaded.")
                return
...
### File: `/plugins/json_tools/__init__.py`
...
### File: `/plugins/json_tools/json_syntax_highlighter.py`
...
### File: `/plugins/json_tools/plugin.json`
...
```json
{
    ...
    "author": "PuffinPy Team",
    "version": "1.0.0",
    "description": "Adds syntax highlighting for JSON files.",
    "entry_point": "plugin_main.py"
}
```

### File: `/plugins/json_tools/plugin_main.py`
...
### File: `/plugins/pythong_tools/code_runner.py`
...
### File: `/plugins/pythong_tools/output_panel.py`
...
### File: `/plugins/pythong_tools/plugin.json`
...
### File: `/plugins/pythong_tools/plugin_main.py`
...
### File: `/plugins/pythong_tools/python_syntax_highlighter.py`
...
### File: `/plugins/rust_tools/plugin.json`
...
```json
{
    ...
    "author": "PuffinPy Team",
    "version": "1.0.0",
    "description": "Adds syntax highlighting support for Rust files (.rs).",
    "entry_point": "plugin_main.py",
    "dependencies": {}
}
```

### File: `/plugins/rust_tools/plugin_main.py`
...
### File: `/plugins/rust_tools/rust_syntax_highlighter.py`
...
### File: `/ui/widgets/list_view_widget.py`

```python
# PuffinPyEditor/ui/widgets/list_view_widget.py
import os
import sys
import subprocess
from PyQt6.QtWidgets import (QWidget, QVBoxLayout, QTreeWidget, QTreeWidgetItem, QInputDialog, QMessageBox,
                             QProxyStyle, QStyle, QApplication, QAbstractItemView, QStyleOptionViewItem)
...
class NoDrawProxyStyle(QProxyStyle):
    """A proxy style whose only job is to prevent the default branch/indicator from being drawn."""
...
class StyledTreeView(QTreeWidget):
    """A custom QTreeWidget that handles its own branch drawing for a modern, two-guide look."""
    ...
    def paintEvent(self, event: QPaintEvent):
        """Override paintEvent to draw the 'Root Guide' on the far left."""
        ...
        if self.topLevelItemCount() > 0:
            ...
            theme = self.theme_manager.current_theme_data.get('colors', {})
            accent_color = QColor(theme.get('accent', '#83c092'))
            painter.setPen(QPen(accent_color, 1))

            root_guide_x = 8
            painter.drawLine(root_guide_x, 0, root_guide_x, self.viewport().height())

    def drawBranches(self, painter: QPainter, rect: QRect, index: QFileInfo):
        """Override to draw perfectly aligned, themed guides with the signature '\' connector."""
        item = self.itemFromIndex(index)
        if not item or not item.parent() or item.parent() == self.invisibleRootItem():
            return

        # --- Theme and Geometry Setup ---
        theme = self.theme_manager.current_theme_data.get('colors', {})
        accent_color = QColor(theme.get('accent', '#83c092'))
        ...
        # --- 1. Draw Vertical Ancestor Lines (Now in Accent Color) ---
        pen = QPen(accent_color, 1)
        painter.setPen(pen)
        for i in range(depth):
            ancestor = item
            for _ in range(depth - i): ancestor = ancestor.parent()
            
            if ancestor.parent() and ancestor.parent().indexOfChild(ancestor) < ancestor.parent().childCount() - 1:
                x = base_offset + (i * indent) + half_indent
                painter.drawLine(QPointF(x, item_rect.top()), QPointF(x, item_rect.bottom()))

        # --- 2. Draw This Item's `\` Connector ---
        is_folder = item.childCount() > 0 or (item.childCount() == 1 and not item.child(0).text(0))
        is_last_child = item.parent().indexOfChild(item) == item.parent().childCount() - 1

        # Set line thickness based on whether it's a folder or file
        pen.setWidthF(2.0 if is_folder else 1.0)
        painter.setPen(pen)
        
        # Define X coordinates for the arrow (left) and the parent's guide (right)
        expander_x = base_offset + (depth * indent) + (indent * 0.25) # Arrow is 1/4 into the slot
        parent_guide_x = base_offset + ((depth - 1) * indent) + half_indent if depth > 0 else base_offset
        center_y = item_rect.center().y()
        diagonal_start_y = center_y - 4.0

        # Draw the vertical line from the parent level
        if depth > 0: # Don't draw for direct children of root
             if is_last_child:
                 painter.drawLine(QPointF(parent_guide_x, item_rect.top()), QPointF(parent_guide_x, diagonal_start_y))
             else:
                 painter.drawLine(QPointF(parent_guide_x, item_rect.top()), QPointF(parent_guide_x, item_rect.bottom()))
        
        # Draw the diagonal `\` connector from the parent's line to this item's arrow
        painter.drawLine(QPointF(parent_guide_x, diagonal_start_y), QPointF(expander_x, center_y))
        
        # --- 3. Draw Expander Icon (Rotating Chevron Style) ---
        if is_folder:
            painter.setPen(QPen(accent_color, 1.2)) # Use a thin pen for the arrow itself
            self._draw_expander_at(painter, QPointF(expander_x, center_y), item.isExpanded())

        painter.restore()

    def _draw_expander_at(self, painter: QPainter, position: QPointF, is_open: bool):
        """Draws a rotating chevron arrow at the given position."""
        arrow_size = 3.5
        p1 = position + QPointF(-arrow_size / 2, -arrow_size)
        p2 = position + QPointF(arrow_size / 2, 0)
        p3 = position + QPointF(-arrow_size / 2, arrow_size)
    ...
    def startDrag(self, supportedActions):
        ...
        if not item or not (data := item.data(0, Qt.ItemDataRole.UserRole)) or not (path := data.get('path')):
            return
    ...
    def dragMoveEvent(self, event):
        ...
        is_invalid_move = (os.path.normpath(source_path) == os.path.normpath(dest_dir) or
                           (os.path.isdir(source_path) and
                            os.path.normpath(dest_dir).startswith(os.path.normpath(source_path) + os.sep)))
    ...
    def keyPressEvent(self, event: QKeyEvent):
        if event.key() == Qt.Key.Key_Delete and (item := self.currentItem()):
...
class FileSystemListView(QWidget):
    ...
    def _schedule_refresh(self, path: str):
        log.debug(f"FS event detected for: {path}. Scheduling refresh.")
    ...
    def refresh(self):
        ...
        self.git_statuses = {}
        project_paths = self.project_manager.get_open_projects()
        for project_path in project_paths:
             self.git_statuses.update(get_git_statuses_for_root(project_path))
             self._add_to_watcher(project_path)
        ...
        for project_path in project_paths:
            ...
            root_item.setIcon(0, qta.icon('fa5s.folder-open', color=self.theme_manager.current_theme_data.get('colors', {}).get('accent')))
    ...
    def _set_expanded_children(self, parent_item, expanded_set):
        for i in range(parent_item.childCount()):
            ...
            if data := child.data(0, Qt.ItemDataRole.UserRole):
                if path := data.get('path'):
    ...
    def _update_tree_after_creation(self, full_path: str):
        parent_dir = os.path.dirname(full_path)
        parent_item = self._find_item_by_path(self.tree_widget.invisibleRootItem(), parent_dir)
        ...
        if parent_item:
            ...
            QTimer.singleShot(50, lambda: self._select_and_scroll_to_path(full_path))
        else:
            self.refresh()
            QTimer.singleShot(50, lambda: self._select_and_scroll_to_path(full_path))
    
    def _select_and_scroll_to_path(self, path):
        new_item = self._find_item_by_path(self.tree_widget.invisibleRootItem(), path)
        if new_item:
            self.tree_widget.setCurrentItem(new_item)
            self.tree_widget.scrollToItem(new_item, QAbstractItemView.ScrollHint.PositionAtCenter)
    ...
    def _find_item_by_path(self, parent_item, path_to_find):
        for i in range(parent_item.childCount()):
            ...
            if child_data and os.path.normpath(child_data.get('path', '')) == os.path.normpath(path_to_find):
                return child
    ...
    def _populate_node(self, parent_item, path):
        ...
        try:
            colors = self.theme_manager.current_theme_data.get('colors', {})
            untracked_color = QColor(colors.get('git.added', '#a7c080'))
            modified_color = QColor(colors.get('git.status.foreground', '#87ceeb'))
            ignored_color = QColor(colors.get('syntax.comment', '#5f6c6d'))

            entries = sorted(os.scandir(path), key=lambda e: (not e.is_dir(), e.name.lower()))
            for entry in entries:
                if entry.name.startswith(('.', '__')) and entry.name not in ['.gitignore', '.gitattributes']:
                    continue
                ...
                status = self.git_statuses.get(os.path.normpath(entry.path))
                if status == '??': item.setForeground(0, untracked_color)
                elif status == 'M': item.setForeground(0, modified_color)
                elif status == '!!': item.setForeground(0, ignored_color)

                if entry.is_dir():
                    item.addChild(QTreeWidgetItem())
                    self._add_to_watcher(entry.path)
        except OSError as e:
    ...
    def on_item_collapsed(self, item):
        data = item.data(0, Qt.ItemDataRole.UserRole)
        if not data or data.get('is_root', False): return

        item.takeChildren()
        item.addChild(QTreeWidgetItem())
        if path := data.get('path'):
            self._remove_from_watcher(path)


    def show_context_menu(self, position):
        item = self.tree_widget.itemAt(position)
        path = item.data(0, Qt.ItemDataRole.UserRole)['path'] if item else None
    ...
    def _action_new_file(self, target_dir: str):
        ...
        if ok and file_name:
            ...
            if success:
                self._update_tree_after_creation(full_path)
                self.api.get_main_window()._action_open_file(full_path)
            else:
    ...
    def _action_delete(self, path_to_delete: str):
        ...
        reply = QMessageBox.question(self, f"Confirm Delete", warning_message,
                                     QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
                                     QMessageBox.StandardButton.No)
        if reply == QMessageBox.StandardButton.Yes:
            success, message = self.file_handler.delete_item(path_to_delete)
            if success:
                parent_path = os.path.dirname(path_to_delete)
                parent_item = self._find_item_by_path(self.tree_widget.invisibleRootItem(), parent_path)
                if parent_item:
                    self._populate_node(parent_item, parent_path)
                else:
                    self.refresh()
            else:
    ...
    def _action_copy_relative_path(self, path: str):
        if not path: return
        
        for proj_path in self.project_manager.get_open_projects():
            if path.startswith(proj_path):
                relative_path = os.path.relpath(path, proj_path)
                QApplication.clipboard().setText(relative_path)
                log.info(f"Copied relative path to clipboard: {relative_path}")
                return

    def _action_open_in_explorer(self, path: str):
        if not path: return
        ...
        try:
            if sys.platform == 'win32':
                ...
                subprocess.run(['open', '-R', os.path.normpath(path)], check=True)
            else:
                subprocess.run(['xdg-open', os.path.normpath(folder)], check=True)
...
class MainWindow(QMainWindow):
    ...
    COMMENT_MAP = { '.py': '#', '.js': '//', '.ts': '//', '.cs': '//', '.java': '//', '.go': '//', '.rs': '//', '.c': '//', '.cpp': '//', '.h': '//', '.hpp': '//', '.css': '/*', '.html': '<!--' }
    END_COMMENT_MAP = {'/*': '*/', '<!--': '-->'}
    ...
    def __init__(self, file_handler, theme_manager, debug_mode=False, parent=None):
        ...
        self._integrate_autopopulate_helper()
        ...
        if self.source_control_panel:
            ...
            if self.explorer_panel:
                self.explorer_panel.tree_widget.currentItemChanged.connect(self.source_control_panel.refresh_all_projects)
    ...
    def _add_new_tab(self, filepath=None, content="", is_placeholder=False):
        ...
        try:
            ...
            if highlighter_class := self.puffin_api.highlighter_map.get(os.path.splitext(filepath or "")[1].lower()):
            ...
            name = os.path.basename(filepath) if filepath else f"Untitled-{self.untitled_file_counter + 1}"
    ...
    def _integrate_source_control_ui(self):
        ...
        self.add_dock_panel(self.source_control_panel, "Source Control", Qt.DockWidgetArea.BottomDockWidgetArea, "fa5b.git-alt")

    def _integrate_global_drag_drop(self):
        self.setAcceptDrops(True)
        self.installEventFilter(self)

    def _integrate_autopopulate_helper(self):
    ...
    def eventFilter(self, obj, event: QEvent) -> bool:
        if obj is self:
            if event.type() == QEvent.Type.DragEnter and event.mimeData().hasUrls():
                ...
                if files:
                    log.info(f"Accepted drop for: {files}")
                    for f in files: self._action_open_file(f)
                    event.acceptProposedAction()
                    return True
        return super().eventFilter(obj, event)

    def _update_problems_panel(self, problems: list):
        from .editor_widget import EditorWidget
    ...
    def _create_core_actions(self):
        actions_map = { "new_file": ("&New File", self._add_new_tab, "Ctrl+N", 'fa5s.file'), "open_file": ("&Open File...", self._action_open_file_dialog, "Ctrl+O", 'fa5s.folder-open'), "open_folder": ("Open &Folder...", self._action_open_folder, "Ctrl+Shift+O", 'fa5s.folder'), "close_project": ("&Close Project", self._action_close_project, None, None), "save": ("&Save", self._action_save_file, "Ctrl+S", 'fa5s.save'), "save_as": ("Save &As...", self._action_save_as, "Ctrl+Shift+S", None), "save_all": ("Save A&ll", self._action_save_all, "Ctrl+Alt+S", None), "find_replace": ("&Find/Replace...", self.toggle_find_panel, "Ctrl+F", "fa5s.search"), "preferences": ("&Preferences...", self._action_open_preferences, "Ctrl+,", 'fa5s.cog'), "exit": ("E&xit", self.close, "Ctrl+Q", None), "force_quit": ("&Force Quit", self._action_force_quit, "Ctrl+Shift+Q", 'fa5s.exclamation-triangle') }
        for key, (text, cb, sc, icon) in actions_map.items():
            ...
            if key == "find_replace": action.setEnabled(False)
    ...
    def _create_core_menu(self):
        ...
        self.file_menu.addAction(self.actions["force_quit"])
    ...
    def _create_toolbar(self):
        ...
        self.main_toolbar.addAction(self.toolbar_find_action)
    ...
    def _apply_theme_and_icons(self, theme_id: str):
        ...
        colors = self.theme_manager.current_theme_data.get('colors', {})
        active_border = colors.get('tab.activeBorderTop', colors.get('tab.activeBorder', '#e06c75'))
        active_bg = colors.get('editor.background', '#1e1e1e')
        active_fg = colors.get('tab.activeForeground', '#ffffff')
        self.tab_widget.setStyleSheet(f"QTabBar::tab:selected {{ background: {active_bg}; color: {active_fg}; border-top: 2px solid {active_border}; border-bottom-color: {active_bg}; }} QTabBar::tab:!selected {{ margin-top: 2px; border-bottom: none; }} QTabWidget::pane {{ border: none; }}")
    ...
    def _rebuild_theme_menu(self):
        ...
        for theme_id, name in self.theme_manager.get_available_themes_for_ui().items():
            action = QAction(name, self, checkable=True, triggered=lambda _, t_id=theme_id: self._on_theme_selected(t_id))
            action.setData(theme_id)
            action.setChecked(theme_id == self.theme_manager.current_theme_id)
            group.addAction(action)
            self.theme_menu.addAction(action)

    def add_dock_panel(self, panel_widget: QWidget, title: str, area: Qt.DockWidgetArea, icon_name: Optional[str] = None):
        if area == Qt.DockWidgetArea.BottomDockWidgetArea:
            if self._bottom_tab_widget is None:
                ...
                super().addDockWidget(area, self._bottom_dock_widget)
                if self.view_menu: self.view_menu.addSeparator(); self.view_menu.addAction(self._bottom_dock_widget.toggleViewAction())
    ...
    def _action_open_file(self, filepath: Optional[str] = None, content: Optional[str] = None):
        ...
        if content is None:
            try:
                with open(norm_path, 'r', encoding='utf-8', errors='replace') as f: content = f.read()
            except Exception as e:
    ...
    def _action_open_file_dialog(self):
        ...
        if error: QMessageBox.critical(self, "Error Opening File", error)
        elif filepath: self._action_open_file(filepath, content)

    def _action_open_folder(self, path: Optional[str] = None):
        if not path or not isinstance(path, str):
            path = QFileDialog.getExistingDirectory(self, "Open Folder", self.project_manager.get_active_project_path() or os.path.expanduser("~"))
    ...
    def _action_close_project(self, path: Optional[str] = None):
        ...
        else: self.statusBar().showMessage("No active project to close.", 2000)
    ...
    def _on_tab_changed(self, index):
        ...
        if is_editor:
            ...
            for i in range(self.tab_widget.count()):
                if (w := self.tab_widget.widget(i)) and isinstance(w, EditorWidget) and w.find_panel.isVisible():
                    w.hide_find_panel()

    def _is_editor_modified(self, editor: QWidget) -> bool:
        from .editor_widget import EditorWidget
        return isinstance(editor, EditorWidget) and (data := self.editor_tabs_data.get(editor)) and hash(editor.get_text()) != data['original_hash']

    def _on_content_changed(self, editor: QWidget):
        from .editor_widget import EditorWidget
        ...
        if index != -1:
            ...
            if is_modified and not current_text.endswith(' *'): self.tab_widget.setTabText(index, current_text + ' *')
            elif not is_modified and current_text.endswith(' *'): self.tab_widget.setTabText(index, current_text[:-2])
        self._update_window_title()
        if self.settings.get("auto_save_enabled", False): self.auto_save_timer.start(self.settings.get("auto_save_delay_seconds", 3) * 1000)

    def _update_window_title(self):
        from .editor_widget import EditorWidget
        base = "PuffinPyEditor"
        proj = os.path.basename(self.project_manager.get_active_project_path()) if self.project_manager.is_project_open() else ""
        current = ""
        widget = self.tab_widget.currentWidget()
        if isinstance(widget, EditorWidget):
            fp = self.editor_tabs_data.get(widget, {}).get('filepath')
            current = os.path.basename(fp) if fp else self.tab_widget.tabText(self.tab_widget.currentIndex()).replace(" *", "")
            if self._is_editor_modified(widget): current += " *"
        self.setWindowTitle(" - ".join(filter(None, [current, proj, base])))

    def _action_save_file(self, editor_widget=None, save_as=False):
        from .editor_widget import EditorWidget
        editor = editor_widget or self.tab_widget.currentWidget()
        if not isinstance(editor, EditorWidget) or not (data := self.editor_tabs_data.get(editor)): return None
        ...
        if fp := self.file_handler.save_file_content(data['filepath'], content, save_as):
            data.update({'filepath': fp, 'original_hash': hash(content)})
            editor.set_filepath(fp)
            if (index := self.tab_widget.indexOf(editor)) != -1:
                self.tab_widget.setTabText(index, os.path.basename(fp))
                self.tab_widget.setTabToolTip(index, fp)
            self.statusBar().showMessage(f"File saved: {os.path.basename(fp)}", 3000)
            self._on_content_changed(editor)
            self.file_handler._add_to_recent_files(fp)
            return fp
        self.statusBar().showMessage("Save cancelled.", 2000)
        return None

    def _action_save_as(self): self._action_save_file(save_as=True)
    def _action_save_all(self):
        if count := sum(1 for i in range(self.tab_widget.count()) if self._is_editor_modified(w := self.tab_widget.widget(i)) and self._action_save_file(w)):
            self.statusBar().showMessage(f"Saved {count} file(s).", 3000)

    def _action_close_tab_by_index(self, index): self._close_widget_safely(self.tab_widget.widget(index), QCloseEvent())
    ...
    def _close_widget_safely(self, widget, event):
        ...
        if self._is_editor_modified(widget):
            name = self.tab_widget.tabText(self.tab_widget.indexOf(widget)).replace(" *", "")
            ret = QMessageBox.question(self, "Save Changes?", f"Save changes to {name}?", QMessageBox.StandardButton.Save | QMessageBox.StandardButton.Discard | QMessageBox.StandardButton.Cancel)
            if ret == QMessageBox.StandardButton.Cancel or (ret == QMessageBox.StandardButton.Save and not self._action_save_file(widget)):
                event.ignore(); return
    ...
    def _update_recent_files_menu(self):
        ...
        for i, filepath in enumerate(recent_files[:10]):
            # This is the correct way to create a QAction with extra properties
    ...
    def _trigger_file_linter(self): pass
    def _show_about_dialog(self): QMessageBox.about(self, "About", f"PuffinPyEditor v{versioning.APP_VERSION}")
    def _open_github_link(self): QDesktopServices.openUrl(QUrl("https://github.com/Stelliro/PuffinPyEditor"))
    ...
    def _on_editor_settings_changed(self):
        from .editor_widget import EditorWidget
        for i in range(self.tab_widget.count()):
            if isinstance(widget := self.tab_widget.widget(i), EditorWidget): widget.apply_styles_and_settings()
    def _action_open_preferences(self):
        if not self.preferences_dialog or not self.preferences_dialog.isVisible():
            self.preferences_dialog = PreferencesDialog(self.git_manager, self.github_manager, self.plugin_manager, self.puffin_api, self)
    ...
    def _action_force_quit(self): log.warning("Force Quit triggered."); QApplication.instance().quit()
    def _goto_definition_result(self, filepath, line, col):
        ...
        norm_path = os.path.normpath(filepath)
        for i in range(self.tab_widget.count()):
            if isinstance(editor := self.tab_widget.widget(i), EditorWidget) and self.editor_tabs_data.get(editor, {}).get('filepath') == norm_path:
                self.tab_widget.setCurrentIndex(i); editor.goto_line_and_column(line, col); return
        self._action_open_file(norm_path);
        if isinstance(current := self.tab_widget.currentWidget(), EditorWidget):
            current.goto_line_and_column(line, col)
        else:
            log.warning(f"Could not jump to definition. Expected editor for {norm_path}.")
    def _shutdown_plugins(self):
        log.info("Shutting down plugins...");
        for plugin in self.plugin_manager.get_loaded_plugins():
            try:
                if hasattr(plugin.instance, 'shutdown'):
                    log.info(f"Shutdown: {plugin.name}")
                    plugin.instance.shutdown()
            except Exception as e: log.error(f"Error shutting down plugin '{plugin.name}': {e}")
    def _shutdown_managers(self):
        log.info("Shutting down core managers..."); [m.shutdown() for m in [self.completion_manager, self.github_manager, self.git_manager, self.linter_manager] if hasattr(m, 'shutdown')]
    def closeEvent(self, event: QCloseEvent):
        ...
        while self.tab_widget.count() > 0:
            ...
            if not dummy_event.isAccepted():
                self._is_app_closing = False; event.ignore(); return
        ...
        self.project_manager.save_session(); self.settings.save()
        self._shutdown_plugins(); self._shutdown_managers()
        log.info("PuffinPyEditor exited cleanly.")
        event.accept()
    def toggle_find_panel(self):
        from .editor_widget import EditorWidget
        if not isinstance(editor := self.tab_widget.currentWidget(), EditorWidget):
            log.warning("Find called on non-editor widget.")
            return
        if editor.find_panel.isVisible(): editor.hide_find_panel()
        else: editor.show_find_panel()
    def _on_active_project_changed(self, current, _):
    ...
    def _handle_new_folder(self, folder_path: str):
        ...
        try:
            ...
            log.error(f"Failed to create __init__.py in {folder_path}: {e}")
    def _generate_header_line(self, file_path: str) -> str | None:
        ext, comment_start = os.path.splitext(file_path)[1].lower(), self.COMMENT_MAP.get(os.path.splitext(file_path)[1].lower())
        ...
        content = f"PuffinPyEditor/{os.path.basename(project_root)}/{rel_path}"; comment_end = self.END_COMMENT_MAP.get(comment_start, "")
    ...
    def _add_header_to_new_file(self, file_path: str):
        ...
        except IOError as e: log.error(f"Failed to write header to {file_path}: {e}")
    def _update_file_header(self, file_path: str):
        if not (new_header := self._generate_header_line(file_path)): return
        try:
            with open(file_path, 'r', encoding='utf-8') as f: lines = f.readlines()
            header_regex = re.compile(r"^(#|//|<!--|\/\*)\s*PuffinPyEditor/.*")
            if not lines: lines.append(new_header)
            elif header_regex.match(lines[0]): lines[0] = new_header
        ...
        except Exception as e: log.error(f"Failed to update header for {file_path}: {e}", exc_info=True)
```
