...
## Export Timestamp: 2025-07-01T11:02:06.249703
---
...
class PluginManager:
    def __init__(self, main_window):
        # MODIFIED: Defer the import to break the circular dependency
...
# --- MODIFICATION: Added 'git.status.foreground' to both built-in themes ---
BUILT_IN_THEMES = {
    "puffin_dark": {"name": "Puffin Dark", "author": "PuffinPy", "type": "dark", "is_custom": False,
                    "colors": {"window.background": "#2f383e", "sidebar.background": "#2a3338",
                               ...
                               "syntax.number": "#d699b6", "tree.indentationGuides.stroke": "#5f6c6d",
                               "tree.trace.color": "#83c092", "git.status.foreground": "#87ceeb"}},
    "puffin_light": {"name": "Puffin Light", "author": "PuffinPy", "type": "light", "is_custom": False,
                     "colors": {"window.background": "#f5f5f5", "sidebar.background": "#ECECEC",
                                ...
                                "syntax.className": "#267f99", "syntax.functionName": "#795e26",
                                "syntax.comment": "#008000", "syntax.string": "#a31515", "syntax.docstring": "#a31515",
                                "syntax.number": "#098658", "tree.indentationGuides.stroke": "#D0D0D0",
                                "tree.trace.color": "#007ACC", "git.status.foreground": "#007ACC"}}
}
...
class CppToolsPlugin(QObject):
    ...
    def _on_tab_changed(self, index):
        ...
        is_cpp_file = False
        if index != -1:
            current_widget = self.main_window.tab_widget.widget(index)
            if (editor_data := self.main_window.editor_tabs_data.get(current_widget)):
...
class CSharpToolsPlugin(QObject):
    ...
    def _on_tab_changed(self, index):
        ...
        if index != -1:
            ...
            if isinstance(widget, EditorWidget) and (data := self.main_window.editor_tabs_data.get(widget)):
...
class FindReplacePlugin:
    def __init__(self, main_window):
        ...
        # Correctly get the API and MainWindow objects
        ...
        # THE FIX: Create the action with the correct parent
        ...
        # THE FIX: Use the API to add the action to the toolbar
        prefs_action = self.main_window.actions.get("preferences")
        main_toolbar = getattr(self.main_window, 'main_toolbar', None)
        if main_toolbar and prefs_action:
            main_toolbar.insertAction(prefs_action, toolbar_find_action)
        else:
            self.api.add_toolbar_action(toolbar_find_action)

        self.find_action.setEnabled(False)
        toolbar_find_action.setEnabled(False)
    ...
    def _on_tab_changed(self, index: int):
        ...
        if hasattr(self, 'toolbar_find_action'):
...
class LinterUIPlugin:
    def __init__(self, puffin_api):
        ...
        # THE FIX: Pass the main_window as the parent, not the API object.
...
# RENAMED from show_explorer_context_menu to show_project_context_menu
def show_project_context_menu(panel, position: QPoint, path: str, is_dir: bool):
    ...
    Includes options for copying file paths and running scripts.
    """
    tree = panel.tree
    # If path is a file, the target dir for 'New' actions is its parent
    target_dir = path if is_dir else os.path.dirname(path)
    is_valid_selection = tree.indexAt(position).isValid()
    # Safely get project_path, will be None for file explorer
    project_path = getattr(panel, 'project_path', None)
    ...
    if is_valid_selection:
        ...
        # Add "Run Script" option for .py files
        if path.lower().endswith('.py') and not is_dir:
            def run_script_action():
                runner_plugin = panel.api.get_plugin_instance("python_runner")
                if runner_plugin:
                    runner_plugin.run_specific_script(path)
                else:
                    QMessageBox.warning(panel.tree, "Plugin Not Found",
                                        "Python Runner plugin is not loaded.")

            menu.addAction(qta.icon('fa5s.play', color='#4CAF50'), "Run Script", run_script_action)
            menu.addSeparator()
        ...
        # Only show "Copy Relative Path" if we are in a project context
        if project_path:
            try:
                ...
                rel_path_action.triggered.connect(
                    lambda: _copy_to_clipboard(relative_path.replace("\\", "/")))
            except ValueError:
                pass

    menu.addSeparator()
    # Use panel's path if no item is selected (for revealing the root folder)
    reveal_path = path if is_valid_selection else project_path or target_dir
...
class DraggableTreeView(QTreeView):
    """
    A QTreeView subclass configured for file and project exploring.
    """
    ...
    def __init__(self, puffin_api: PuffinPluginAPI, parent=None):
        ...
        # --- Column and Header Configuration ---
        header = self.header()
        ...
        header.setSectionResizeMode(3, QHeaderView.ResizeMode.Interactive)  # Date column
        # Show all relevant columns by default
        header.setSectionHidden(2, False)  # Show 'Type'

        self.update_stylesheet()
        main_window = self.api.get_main_window()
        if main_window:
            main_window.theme_changed_signal.connect(self.update_stylesheet)
    ...
    def update_stylesheet(self, theme_id: str = ""):
        ...
        text_color = colors.get('editor.foreground', '#cccccc')
        selection_bg = colors.get('accent', '#d35400')
        selection_fg = colors.get('button.foreground', '#ffffff')
        base_bg = colors.get('editor.background', '#2b2b2b')
        alt_bg = colors.get('editor.lineHighlightBackground', '#323232')
        ...
        self.setStyleSheet(f"""
            ...
            QHeaderView::section {{
                background-color: {colors.get('sidebar.background', '#3c3f41')};
                border: 1px solid {colors.get('input.border', '#555555')};
                padding: 4px;
            }}
            QTreeView::item {{
                height: 24px;
...
### File: `/plugins/project_explorer/file_system_explorer_panel.py`

```python
# PuffinPyEditor/plugins/project_explorer/file_system_explorer_panel.py
import os
from PyQt6.QtWidgets import (QWidget, QVBoxLayout, QFileSystemModel, QComboBox,
                             QHBoxLayout, QLabel, QInputDialog, QMessageBox, QLineEdit)
from PyQt6.QtCore import QDir, QPoint, Qt
from PyQt6.QtGui import QFileSystemModel
from app_core.puffin_api import PuffinPluginAPI
from .draggable_tree import DraggableTreeView
# REMOVED: No longer using the custom delegate
...
class FileSystemExplorerPanel(QWidget):
    """
    A widget that provides a general-purpose view of the computer's file system,
    allowing users to browse any directory.
    """
    ...
    def __init__(self, puffin_api: PuffinPluginAPI, parent=None):
        ...
        self.path_selector.setEditable(True)
        self._populate_path_selector()
        ...
        self.tree.model().setIconProvider(CustomFileIconProvider(self.api))

        # REMOVED: The delegate is no longer used
        # self.delegate = StyledIndentDelegate(self.tree, self.api, self)
        ...
        self.path_selector.currentIndexChanged.connect(self._on_path_selected)

        self.tree.setRootIndex(self.model.index(QDir.homePath()))
        self.path_selector.setCurrentText(QDir.homePath())

    def on_theme_changed(self, new_icon_provider):
        """Receives the new icon provider from the parent plugin."""
        if hasattr(self, 'model'):
            self.model.setIconProvider(new_icon_provider)
            self.tree.viewport().update()

    def _populate_path_selector(self):
        self.path_selector.addItem(QDir.homePath())
        for drive in QDir.drives():
            self.path_selector.addItem(drive.absolutePath())

    def _on_path_selected(self, index):
        path = self.path_selector.itemText(index)
    ...
    def _on_item_double_clicked(self, index):
        if not self.model.isDir(index):
            ...
            self.api.get_main_window()._action_open_file(file_path)
    ...
    def _action_new_file(self, target_dir: str):
        name, ok = QInputDialog.getText(self, "New File", "Enter file name:", QLineEdit.EchoMode.Normal, "")
        if ok and name: self.file_handler.create_file(os.path.join(target_dir, name))

    def _action_new_folder(self, target_dir: str):
        name, ok = QInputDialog.getText(self, "New Folder", "Enter folder name:", QLineEdit.EchoMode.Normal, "")
        if ok and name: self.file_handler.create_folder(os.path.join(target_dir, name))
    ...
    def _action_rename(self, old_path: str):
        ...
        if ok and new_name and new_name != old_name:
            success, result = self.file_handler.rename_item(old_path, new_name)
            if not success: QMessageBox.critical(self, "Rename Error", result)

    def _action_delete(self, path: str):
        item_type = 'folder' if os.path.isdir(path) else 'file'
        reply = QMessageBox.question(self, "Confirm Delete", f"Delete the {item_type} '{os.path.basename(path)}'?",
                                     QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
                                     QMessageBox.StandardButton.No)
        if reply == QMessageBox.StandardButton.Yes: self.file_handler.delete_item(path)

    def _action_duplicate(self, path: str):
        success, message = self.file_handler.duplicate_item(path)
        if not success: QMessageBox.warning(self, "Duplicate Error", message)
```

### File: `/plugins/project_explorer/folder_size_worker.py`

```python
# PuffinPyEditor/plugins/project_explorer/folder_size_worker.py
import os
...
class WorkerSignals(QObject):
    ...
    - finished: Emits the path and its calculated size in bytes.
    """
...
class FolderSizeWorker(QRunnable):
    """
    A QRunnable worker that calculates the total size of a directory
    recursively in the background.
    """
    ...
    def run(self):
        """The main work of the thread. Calculates size and emits result."""
        total_size = 0
        try:
            for dirpath, _, filenames in os.walk(self.path, topdown=True, onerror=None):
                if self.is_cancelled:
                    # Use -2 to indicate cancellation
...
class CustomFileIconProvider(QFileIconProvider):
    ...
    # Map of file/folder names and extensions to qtawesome icon names. (Corrected)
    ICON_MAP = {
        # --- Special Folder Names ---
        ".git": "mdi.git",
        "__pycache__": "mdi.folder-cog-outline",
        "venv": "mdi.folder-cog-outline", ".venv": "mdi.folder-cog-outline",
        "dist": "mdi.folder-zip-outline", "build": "mdi.folder-zip-outline",
        ...
        ".vscode": "mdi.folder-vscode",
        ".idea": "mdi.folder-cog-outline",  # No specific jetbrains folder icon exists

        # --- Special File Names ---
        "Dockerfile": "mdi.docker", ".dockerignore": "mdi.docker", "docker-compose.yml": "mdi.docker",
        ".gitignore": "mdi.git", ".gitattributes": "mdi.git",
        "pyproject.toml": "mdi.language-python", "poetry.lock": "mdi.language-python",
        "requirements.txt": "mdi.format-list-numbered",
        "package.json": "mdi.npm", "package-lock.json": "mdi.npm", "pnpm-lock.yaml": "mdi.npm", "yarn.lock": "mdi.npm",
        ".env": "mdi.key-variant", ".env.example": "mdi.key-outline",  # Corrected key icons
        "README.md": "mdi.book-open-variant",

        # --- Code & Programming ---
        ".py": "mdi.language-python", ".pyc": "mdi.language-python", ".pyw": "mdi.language-python",
        ".js": "mdi.language-javascript", ".mjs": "mdi.language-javascript",
        ".ts": "mdi.language-typescript", ".tsx": "mdi.language-typescript",
        ".java": "mdi.language-java", ".jar": "mdi.language-java",
        ".cs": "mdi.language-csharp", ".csproj": "mdi.language-csharp",
        ".cpp": "mdi.language-cpp", ".hpp": "mdi.language-cpp",
        ...
        # --- Web ---
        ".html": "mdi.language-html5", ".htm": "mdi.language-html5",
        ".css": "mdi.language-css3", ".scss": "mdi.language-css3",
        ".json": "mdi.code-json", ".xml": "mdi.xml",
        ".yaml": "mdi.yaml", ".yml": "mdi.yaml",
        ".toml": "mdi.cog-outline", ".ini": "mdi.cog-outline", ".cfg": "mdi.cog-outline", ".conf": "mdi.cog-outline",

        # --- Databases ---
        ".sql": "mdi.database", ".db": "mdi.database", ".sqlite3": "mdi.database",

        # --- Documents & Text ---
        ".md": "mdi.markdown", ".txt": "mdi.file-document-outline", ".log": "mdi.file-document-outline",
        ".pdf": "mdi.file-pdf-box", ".rtf": "mdi.file-word", ".docx": "mdi.file-word",
        ".csv": "mdi.file-delimited-outline", ".xls": "mdi.file-excel", ".xlsx": "mdi.file-excel",

        # --- Archives ---
        ".zip": "mdi.folder-zip-outline", ".rar": "mdi.folder-zip-outline", ".7z": "mdi.folder-zip-outline",
        ".tar": "mdi.folder-zip-outline", ".gz": "mdi.folder-zip-outline", ".bz2": "mdi.folder-zip-outline",

        # --- Images & Media ---
        ".png": "mdi.file-image", ".jpg": "mdi.file-image", ".jpeg": "mdi.file-image",
        ".gif": "mdi.file-image", ".bmp": "mdi.file-image", ".ico": "mdi.file-image",
        ".svg": "mdi.svg",
    }
    ...
    def icon(self, fileInfoOrType):
        ...
        main_window = self.api.get_main_window()
        if not main_window:
            return qta.icon('fa5s.file')  # Fallback if API isn't ready
        ...
        # Create extension with dot, e.g., ".txt"
        extension = f".{file_info.suffix().lower()}"

        # 1. Check for exact filename match (e.g., "Dockerfile", "README.md")
        if file_name in self.ICON_MAP:
            icon_name = self.ICON_MAP[file_name]
            color = icon_colors.get(file_name, default_file_color)
            return qta.icon(icon_name, color=color)

        # 2. Check if it's a known binary type to avoid slow OS lookups
        ...
        #    DO NOT call super().icon(), as that is the source of performance issues.
...
```json
{
  ...
  "version": "2.2.0",
  "author": "PuffinPy-Team",
  "description": "A feature-rich file system explorer with Git integration, background tasks, and a custom view.",
  "entry_point": "plugin_main.py",
  "dependencies": {},
  "supported_os": [
    "windows",
...
import os
from PyQt6.QtWidgets import QWidget, QVBoxLayout, QTabWidget, QPushButton, QStackedWidget, QHBoxLayout
...
from .file_system_explorer_panel import FileSystemExplorerPanel
from .icon_provider import CustomFileIconProvider


class ExplorerPlugin:  # RENAMED class
    """
    Manages the explorer dock widget. It hosts a stacked widget to switch
    between a 'Projects' view and a general 'File System' view.
    """
    ...
    def __init__(self, puffin_api: PuffinPluginAPI):
        ...
        self.container_layout.setContentsMargins(0, 0, 5, 5)
        self.container_layout.setSpacing(5)

        self.toolbar = QHBoxLayout()
        self.projects_button = self._create_toolbar_button("Projects", 'fa5s.folder-open', "Show Project Explorer")
        self.filesystem_button = self._create_toolbar_button("File System", 'fa5s.desktop', "Show File System Explorer")

        self.projects_button.setCheckable(True)
        self.filesystem_button.setCheckable(True)
        self.projects_button.setChecked(True)

        self.toolbar.addWidget(self.projects_button)
        self.toolbar.addWidget(self.filesystem_button)
        self.toolbar.addStretch()
        self.container_layout.addLayout(self.toolbar)

        self.view_stack = QStackedWidget()
        self.project_panel = ProjectExplorerPanel(self.api, self.project_manager)
        self.file_system_panel = FileSystemExplorerPanel(self.api)

        self.view_stack.addWidget(self.project_panel)
        self.view_stack.addWidget(self.file_system_panel)
        self.container_layout.addWidget(self.view_stack)

        self.api.register_dock_panel(self.container, "Explorer", Qt.DockWidgetArea.LeftDockWidgetArea, "fa5s.compass")

        # --- Signal Connections ---
        self.projects_button.clicked.connect(
            lambda: self.view_stack.setCurrentIndex(0) or self.projects_button.setChecked(
                True) or self.filesystem_button.setChecked(False))
        self.filesystem_button.clicked.connect(
            lambda: self.view_stack.setCurrentIndex(1) or self.filesystem_button.setChecked(
                True) or self.projects_button.setChecked(False))
        self.project_manager.projects_changed.connect(self.project_panel.refresh_project_views)
        self.main_window.theme_changed_signal.connect(self.on_theme_changed)

        self.project_panel.refresh_project_views()
        log.info("Project Explorer Plugin initialized with stacked view.")

    def _create_toolbar_button(self, text, icon_name, tooltip):
        button = QPushButton(f" {text}")
        button.setIcon(qta.icon(icon_name, color='grey'))
        button.setToolTip(tooltip);
        button.setFlat(True);
    ...
    def on_theme_changed(self, theme_id: str):
        log.info("Explorer: Theme changed, updating icons.")
        new_icon_provider = CustomFileIconProvider(self.api)
        self.projects_button.setIcon(qta.icon('fa5s.folder-open', color='grey'))
        self.filesystem_button.setIcon(qta.icon('fa5s.desktop', color='grey'))
        self.project_panel.on_theme_changed(new_icon_provider)
        self.file_system_panel.on_theme_changed(new_icon_provider)


# This is the required entry point function for the plugin manager.
def initialize(puffin_api: PuffinPluginAPI):
    return ExplorerPlugin(puffin_api)
```
...
import os
from PyQt6.QtWidgets import (QWidget, QVBoxLayout, QTreeView, QMessageBox, QInputDialog,
...
class ProjectExplorerPanel(QWidget):
    """
    A widget that contains a QTabWidget where each tab is a DraggableTreeView
    for an open project.
    """

    def __init__(self, puffin_api: PuffinPluginAPI, project_manager: ProjectManager, parent=None):
        super().__init__(parent)
        self.api = puffin_api
        self.project_manager = project_manager
        ...
        self.project_tabs.currentChanged.connect(self._on_active_project_changed)

    def on_theme_changed(self, new_icon_provider):
        """Receives the new icon provider from the parent plugin."""
        for i in range(self.project_tabs.count()):
            panel = self.project_tabs.widget(i)
            if hasattr(panel, 'model'):
                panel.model.setIconProvider(new_icon_provider)
                panel.viewport().update()

    def _handle_tab_close(self, index: int):
        tree_view = self.project_tabs.widget(index)
        project_path = getattr(tree_view, 'project_path', None)
        if project_path:
            self.api.get_main_window()._action_close_project(project_path)

    def _on_active_project_changed(self, index: int):
        if index != -1:
            tree_view = self.project_tabs.widget(index)
            project_path = getattr(tree_view, 'project_path', None)
            if project_path:
                self.project_manager.set_active_project(project_path)
                self.api.get_main_window()._update_window_title()

    def refresh_project_views(self):
        log.info("Refreshing project explorer views...")
        open_projects = self.project_manager.get_open_projects()
        current_tab_paths = {
            self.project_tabs.widget(i).project_path
            for i in range(self.project_tabs.count())
            if hasattr(self.project_tabs.widget(i), 'project_path')
        }

        for i in range(self.project_tabs.count() - 1, -1, -1):
            panel = self.project_tabs.widget(i)
            if hasattr(panel, 'project_path') and panel.project_path not in open_projects:
                self.project_tabs.removeTab(i)

        for path in open_projects:
            if path not in current_tab_paths:
                tree_view = SingleProjectView(self.api, path)
                index = self.project_tabs.addTab(tree_view, os.path.basename(path))
                self.project_tabs.setTabToolTip(index, path)

        active_path = self.project_manager.get_active_project_path()
        for i in range(self.project_tabs.count()):
            panel = self.project_tabs.widget(i)
            if hasattr(panel, 'project_path') and panel.project_path == active_path:
                self.project_tabs.setCurrentIndex(i)
                break

        if self.project_tabs.count() == 0:
            log.warning("No projects are open. Project Explorer is empty.")


class SingleProjectView(DraggableTreeView):
    """
    A specialized view for ONE project.
    """

    def __init__(self, puffin_api, project_path):
        super().__init__(puffin_api, self)
        self.api = puffin_api
        self.file_handler = self.api.get_manager("file_handler")
        self.project_path = project_path
        ...
        # REMOVED: The delegate is no longer used
        # self.delegate = StyledIndentDelegate(self, self.api, self)
        ...
        self.customContextMenuRequested.connect(self.show_context_menu_wrapper)
...
# REMOVED: No longer need QTransform
from PyQt6.QtGui import QPainter, QPen, QColor, QFontMetrics, QPalette
from PyQt6.QtWidgets import (QApplication, QStyledItemDelegate, QTreeView,
...
def _format_size(size_bytes: int) -> str:
    """Formats size in bytes to a human-readable string (KiB, MiB, etc.)."""
...
@lru_cache(maxsize=1)
def _get_git_status(project_root: str) -> dict:
    """
    Executes 'git status' and returns a dictionary mapping file paths to their status.
    Uses LRU cache to only run this once per paint cycle.
    """
    ...
    try:
        ...
        for line in result.strip().split('\n'):
            ...
            status_chars, path = line[:2], line[3:]
...
class StyledIndentDelegate(QStyledItemDelegate):
    def __init__(self, tree_view: QTreeView, puffin_api, panel):
        super().__init__(tree_view)
        self.tree = tree_view
        self.api = puffin_api
        self.panel = panel
        # REMOVED: Diagonal constants are no longer needed
        # self.DIAGONAL_SHEAR = -0.25
        # self.DIAGONAL_OFFSET_PER_ROW = 5
    ...
    def paint(self, painter: QPainter, option: QStyleOptionViewItem, index: QModelIndex):
        ...
        # --- Reverted to standard style option setup ---
        style_option = QStyleOptionViewItem(option)
        self.initStyleOption(style_option, index)

        model = index.model()
        is_selected = style_option.state & QStyle.StateFlag.State_Selected
        filename = model.fileName(index)
        filepath = model.filePath(index)
        icon = model.iconProvider().icon(model.fileInfo(index))

        project_path_for_git = getattr(self.panel, 'project_path', None)
        git_status = _get_git_status(project_path_for_git).get(
            os.path.normpath(filepath)) if project_path_for_git else None

        theme_mgr = self.api.get_main_window().theme_manager
        colors = theme_mgr.current_theme_data.get('colors', {})

        # --- Color Logic ---
        text_color = style_option.palette.color(QPalette.ColorRole.Text)
        if git_status: text_color = QColor(colors.get('git.status.foreground', '#87ceeb'))
        if is_selected:
            text_color = style_option.palette.color(QPalette.ColorRole.HighlightedText)
            bg_color = style_option.palette.color(QPalette.ColorRole.Highlight)
        else:
            # Check for alternating row colors
            bg_color = option.palette.color(
                QPalette.ColorRole.AlternateBase) if self.tree.alternatingRowColors() and index.row() % 2 != 0 else option.palette.color(
                QPalette.ColorRole.Base)

        anno_color = QColor(text_color);
        anno_color.setAlpha(160)

        # --- Drawing Background ---
        painter.fillRect(option.rect, bg_color)

        # --- Standard Primitive and Indentation Drawing ---
        QApplication.style().drawPrimitive(QStyle.PrimitiveElement.PE_IndicatorBranch, style_option, painter, self.tree)
        self.paint_indentation_guides(painter, style_option, index)

        # --- NEW & IMPROVED Layout Calculation ---
        # The content rect is the item's full rect, indented
        content_rect = self.tree.visualRect(index)

        fm = QFontMetrics(style_option.font)

        # Right-aligned metadata
        metadata_text = self.get_metadata_text(index)
        metadata_width = fm.horizontalAdvance(metadata_text)
        metadata_rect = QRect(
            option.rect.right() - metadata_width - 8,  # 8px padding from right edge
            content_rect.y(),
            metadata_width,
            content_rect.height()
        )
        ...
        git_status_char = (git_status[0] if git_status[0] != '?' else 'U') if git_status else ''
        git_char_width = fm.horizontalAdvance(git_status_char) + 5 if git_status_char else 0
        git_char_rect = QRect(
            metadata_rect.left() - git_char_width,
            content_rect.y(),
            git_char_width,
            content_rect.height()
        )
        ...
        # Icon
        icon_size = style_option.decorationSize
        icon_y_offset = (content_rect.height() - icon_size.height()) // 2
        icon_rect = QRect(content_rect.left(), content_rect.top() + icon_y_offset, icon_size.width(),
                          icon_size.height())

        # Filename (takes up remaining space)
        filename_x = icon_rect.right() + 5
        available_filename_width = git_char_rect.left() - filename_x - 5
        elided_filename = fm.elidedText(filename, Qt.TextElideMode.ElideRight, available_filename_width)

        filename_rect = QRect(filename_x, content_rect.y(), available_filename_width, content_rect.height())

        # --- Painting the content ---
        icon.paint(painter, icon_rect)
        painter.setPen(text_color)
        painter.drawText(filename_rect, int(Qt.AlignmentFlag.AlignVCenter), elided_filename)

        if git_status_char:
            painter.drawText(git_char_rect, int(Qt.AlignmentFlag.AlignVCenter), git_status_char)

        painter.setPen(anno_color)
        painter.drawText(metadata_rect, int(Qt.AlignmentFlag.AlignVCenter | Qt.AlignmentFlag.AlignRight), metadata_text)

        painter.restore()

    def get_metadata_text(self, index: QModelIndex) -> str:
        model = index.model()
        date_str, size_str = "", ""
        if (date := model.lastModified(index)).isValid(): date_str = date.toString("MMM dd hh:mm")
        if hasattr(self.panel, 'folder_size_cache') and model.isDir(index):
            folder_path = model.filePath(index)
            if (size_val := self.panel.folder_size_cache.get(folder_path)) is None:
                if hasattr(self.panel, 'calculate_folder_size'): self.panel.calculate_folder_size(folder_path)
            elif isinstance(size_val, int) and size_val >= 0:
                size_str = _format_size(size_val)
        elif not model.isDir(index):
            size_str = _format_size(model.size(index))
        return f"{size_str:<10}{date_str}"

    def paint_indentation_guides(self, painter: QPainter, option: QStyleOptionViewItem, index: QModelIndex):
        theme_mgr = self.api.get_main_window().theme_manager
        guide_color = QColor(
            theme_mgr.current_theme_data.get('colors', {}).get('tree.indentationGuides.stroke', '#555555'))
        painter.setPen(QPen(guide_color))
        indent = self.tree.indentation()

        temp_index = index
        while temp_index.parent().isValid():
            parent_index = temp_index.parent()
            # We only draw a line if the parent has more items after this one's ancestor
            if parent_index.model().rowCount() > temp_index.row() + 1:
                x_pos = self.tree.visualRect(parent_index).x() + indent // 2
                painter.drawLine(x_pos, option.rect.top(), x_pos, option.rect.bottom())
...
from PyQt6.QtGui import QTextCursor, QFont, QPainter, QColor, QTextFormat, QTextDocument, QSyntaxHighlighter
...
class EditorWidget(QWidget):
    ...
    def __init__(self, completion_manager=None, parent=None):
        ...
        # Highlighter is now dynamic and set after initialization
    ...
    def set_highlighter(self, highlighter_class):
        """
        Sets the syntax highlighter for the editor's document.
        Args:
            highlighter_class: The QSyntaxHighlighter class to instantiate.
        """
...
class PreferencesDialog(QDialog):
    ...
    # [RESTORED] Full method implementation
