...
## Export Timestamp: 2025-07-01T16:33:29.676678
---
...
def show_project_context_menu(panel, position: QPoint, path: str, is_dir: bool):
    ...
    """
    tree = panel.list_widget if hasattr(panel, 'list_widget') else panel.tree
...
from app_core.puffin_api import PuffinPluginAPI
from utils.logger import log
# MODIFIED: Correctly import the now-local helper files
...
class FileListItemWidget(QWidget):
    """A custom widget for displaying a single file or folder in the list."""

    def __init__(self, data, icon_provider, git_status, theme_colors, parent=None):
        super().__init__(parent)
        self.path = data['path']
        ...
        indentation = QWidget()
        indentation.setFixedWidth(25 * data.get('depth', 0))
        layout.addWidget(indentation)

        # Icon
        icon_label = QLabel()
        icon_label.setPixmap(icon_provider.icon(QFileInfo(self.path)).pixmap(QSize(16, 16)))
        ...
        if git_status:
            git_color = theme_colors.get('git.status.foreground', '#87ceeb')
            name_label.setStyleSheet(f"color: {git_color};")

        name_layout.addWidget(name_label)
        name_layout.addStretch()

        if git_status:
            git_label = QLabel(git_status)
            git_label.setStyleSheet(f"color: {git_color}; font-weight: bold;")
            name_layout.addWidget(git_label)

        layout.addLayout(name_layout, 1)

        # Metadata
        meta_color = QColor(theme_colors.get('syntax.comment', '#808080'))
        meta_stylesheet = f"color: {meta_color.name()}; padding-left: 15px;"

        size_label = QLabel(data['size_str']);
        size_label.setMinimumWidth(70);
        size_label.setAlignment(Qt.AlignmentFlag.AlignRight)
        size_label.setStyleSheet(meta_stylesheet)

        type_label = QLabel(data['type_str']);
        type_label.setMinimumWidth(90);
        type_label.setAlignment(Qt.AlignmentFlag.AlignRight)
        type_label.setStyleSheet(meta_stylesheet)

        date_label = QLabel(data['date_str']);
        date_label.setMinimumWidth(110);
        date_label.setAlignment(Qt.AlignmentFlag.AlignRight)
        date_label.setStyleSheet(meta_stylesheet)
...
class FileSystemListView(QWidget):
    def __init__(self, puffin_api: PuffinPluginAPI, path: str, parent: None = None):
        super().__init__(parent)
        self.api = puffin_api
        self.path = path
        self.items_data = []
    ...
    def refresh(self):
        self.items_data = []
        self._scan_directory(self.path, 0)
        self._populate_list()

    def _scan_directory(self, base_path, depth):
        try:
            entries = sorted(os.scandir(base_path), key=lambda e: (e.is_file(), e.name.lower()))
            for entry in entries:
                ...
                try:
                    ...
                    self.items_data.append({
                        'path': entry.path, 'name': entry.name, 'is_dir': is_dir, 'depth': depth,
                        'size': stats.st_size if not is_dir else -1,
                        'date': datetime.fromtimestamp(stats.st_mtime),
                        'type_str': "Folder" if is_dir else self._get_file_type(entry.name),
                        'size_str': "" if is_dir else format_size(stats.st_size),
                        'date_str': datetime.fromtimestamp(stats.st_mtime).strftime("%b %d %H:%M"),
                    })
                    if is_dir:
            ...
            log.warning(f"Could not scan directory: {base_path} - {e}")

    def _get_file_type(self, filename):
        ext = os.path.splitext(filename)[1][1:].lower()
        type_map = {
            'py': 'Python', 'js': 'JS', 'html': 'HTML', 'css': 'CSS', 'md': 'Markdown', 'json': 'JSON', 'txt': 'Text',
        }
        return type_map.get(ext, f"{ext.upper() if ext else 'File'}")

    def _populate_list(self):
        self.list_widget.clear()
        git_statuses = get_git_statuses_for_root(self.path)
        theme_colors = self.theme_manager.current_theme_data.get("colors", {})

        for data in self.items_data:
            list_item = QListWidgetItem(self.list_widget)
            list_item.setSizeHint(QSize(0, 26))

            status = git_statuses.get(os.path.normpath(data['path']))
            git_status_char = status if status else ''

            item_widget = FileListItemWidget(data, self.icon_provider, git_status_char, theme_colors)
    ...
    def on_item_double_clicked(self, item):
        ...
        if data and not data['is_dir']:
            self.main_window._action_open_file(data['path'])
```
...
import os
from PyQt6.QtWidgets import QWidget, QVBoxLayout, QHBoxLayout, QTabWidget, QPushButton, QStackedWidget
...
from .list_view_widget import FileSystemListView  # Corrected import
...
class ListViewExplorerPlugin:
    def __init__(self, puffin_api: PuffinPluginAPI):
        ...
        # Removed the toolbar and stack widget, we will only show projects for now.
        ...
        # --- Signal Connections ---
        self.project_manager.projects_changed.connect(self.refresh_project_views)
        self.tab_widget.tabCloseRequested.connect(self._handle_tab_close)
        self.tab_widget.currentChanged.connect(self._on_active_project_changed)

        self.refresh_project_views()
        log.info("List View Explorer initialized.")

    def refresh_project_views(self):
        open_projects = self.project_manager.get_open_projects()
        current_paths = {self.tab_widget.widget(i).path for i in range(self.tab_widget.count()) if
                         hasattr(self.tab_widget.widget(i), 'path')}

        for i in range(self.tab_widget.count() - 1, -1, -1):
            if self.tab_widget.widget(i).path not in open_projects:
                self.tab_widget.removeTab(i)

        for path in open_projects:
            if path not in current_paths:
                view = FileSystemListView(self.api, path)
                index = self.tab_widget.addTab(view, os.path.basename(path))
                self.tab_widget.setTabToolTip(index, path)

        active_path = self.project_manager.get_active_project_path()
        for i in range(self.tab_widget.count()):
            widget = self.tab_widget.widget(i)
            if hasattr(widget, 'path') and widget.path == active_path:
                self.tab_widget.setCurrentIndex(i)
                break

    def _handle_tab_close(self, index: int):
        view = self.tab_widget.widget(index)
        if hasattr(view, 'path'):
            self.api.get_main_window()._action_close_project(view.path)

    def _on_active_project_changed(self, index: int):
        if index != -1:
            view = self.tab_widget.widget(index)
            if hasattr(view, 'path'):
                self.project_manager.set_active_project(view.path)

    def shutdown(self):
        if self.dock_widget:
            self.main_window.removeDockWidget(self.dock_widget)
            self.dock_widget.deleteLater()
        log.info("List View Explorer shutdown.")
