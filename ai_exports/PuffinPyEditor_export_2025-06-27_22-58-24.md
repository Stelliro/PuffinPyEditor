...
## Export Timestamp: 2025-06-27T22:58:24.583559
---
...
- L436 (W292) No message available
...
def get_arrow_svg_uri(direction: str, color: str) -> str:
    ...
    svg_content = (
        ...
        'viewBox="0 0 16 16">'
        f'<path stroke="{color}" stroke-width="2" stroke-linecap="round" '
        f'fill="none" d="{path_data}" />'
        '</svg>'
    )
...
- L12 (E401) No message available

- L15 (E501) No message available

- L20 (F841) No message available
...
class BadStyle:
    ...
    # This is a runtime error, but not a fatal syntax error, so it should be detected.
...
- L113 (W292) No message available
...
class BreakpointArea(QWidget):
    ...
    def paintEvent(self, event: QPaintEvent):
        ...
        top = int(
            text_area.blockBoundingGeometry(block)
            .translated(text_area.contentOffset()).top()
        )
...
- L257 (E501) No message available
...
- L402 (E501) No message available

- L406 (W292) No message available
...
class CustomPlainTextEdit(QPlainTextEdit):
    ...
    def keyPressEvent(self, event: QKeyEvent) -> None:
        if event.key() == Qt.Key.Key_Print:
            event.ignore()
            log.debug("Print Screen key ignored in editor for OS handling.")
            return
        ...
        if key == Qt.Key.Key_Tab or key == Qt.Key.Key_Backtab:
            ...
            # Note: We don't handle shift-tab (outdent) for a single line here,
            # as it's not a standard behavior for most editors.
            return
    ...
    def _indent_selection(self, indent: bool = True):
        ...
        end_block = (
            cursor.blockNumber() if cursor.columnNumber() != 0 or end == start
            else cursor.blockNumber() - 1
        )

        cursor.beginEditBlock()
        width = settings_manager.get("indent_width")
        indent_str = (
            " " * width if settings_manager.get("indent_style") == "spaces"
            else "\t"
        )
...
class EditorWidget(QWidget):
    ...
    def __init__(self, completion_manager: CompletionManager, parent=None):
        ...
        self.text_area.definition_requested_from_menu.connect(
            self.request_definition_from_context
        )
        ...
        self.text_area.cursor_position_changed_signal.connect(
            self.cursor_position_display_updated.emit
        )
    ...
    def _update_dynamic_highlights(self):
        ...
        current_line_color = QColor(
            colors.get('editor.lineHighlightBackground', '#ffffff')
        )
        ...
        for line_num in self.breakpoints:
            ...
            bp_selection.format.setProperty(
        ...
        while block.isValid() and block.isVisible():
            ...
            if settings_manager.get("indent_style") == "tabs":
                indent_level = text.count('\t') + (
                    leading_spaces // tab_width_setting
                )
            else:
    ...
    def request_signature(self, line: int, col: int):
        if not self.filepath:
            return
        self.completion_manager.request_signature(
            self.get_text(), line, col, self.filepath
        )
    ...
    def replace_current(
        self, query: str, replace_text: str, flags: QTextDocument.FindFlag
    ) -> bool:
        cursor = self.text_area.textCursor()
        if cursor.hasSelection():
    ...
    def line_number_area_paint_event(self, event):
        ...
        current_line_fg_color = QColor(
            colors.get('editor.foreground', '#111111')
        )
        ...
        block_number = block.blockNumber()
        top = int(self.text_area.blockBoundingGeometry(block).translated(
        ...
        while block.isValid() and top <= event.rect().bottom():
            if block.isVisible() and bottom >= event.rect().top():
                number = str(block_number + 1)
                is_current = (block_number == current_line_number)
                painter.setPen(current_line_fg_color if is_current else fg_color)
                painter.drawText(
    ...
    def update_editor_settings(self):
        ...
        font = QFont(
            settings_manager.get("font_family"), settings_manager.get("font_size")
        )
...
- L131 (W292) No message available
...
class FileTreeViewWidget(QTreeView):
    ...
    def _show_context_menu(self, position: QPoint):
        ...
        clicked_path = (self.file_system_model.filePath(index)
                        if index.isValid()
                        else self.file_system_model.rootPath())
        if not clicked_path:
            return

        target_dir = (clicked_path if os.path.isdir(clicked_path)
                      else os.path.dirname(clicked_path))

        menu.addAction(qta.icon('fa5s.file-alt'), "New File...",
                       partial(self._action_new_file, target_dir))
        menu.addAction(qta.icon('fa5s.folder-plus'), "New Folder...",
    ...
    def _action_new_folder(self, base_path: str):
        ...
        if ok and name:
            success, error = self.file_handler.create_folder(
...
- L429 (E501) No message available

- L430 (E501) No message available

- L431 (E501) No message available
...
- L535 (E501) No message available

- L537 (E501) No message available

- L574 (E501) No message available

- L580 (E501) No message available

- L584 (E501) No message available

- L598 (E501) No message available

- L620 (E501) No message available

- L626 (E501) No message available

- L750 (E501) No message available

- L769 (W292) No message available
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, debug_mode: bool = False):
        ...
        # Note: Inter-plugin signal connections are no longer handled here.
        # Plugins should use the PuffinAPI to find other plugins and connect
        # to them during their own initialization.
    ...
    def _on_project_tab_changed(self, index: int):
        ...
        # The Source Control UI plugin, if present, should listen for a signal
        # from the project manager or main window to know when to refresh.
        # This removes hardcoded dependencies from the main window.
        self._update_window_title()
    ...
    def _update_window_title(self):
        ...
        editor = self.tab_widget.currentWidget()
        if isinstance(editor, EditorWidget) and editor in self.editor_tabs_data:
    ...
    def _action_open_file(self, filepath: str, content: Optional[str] = None):
        ...
        for i in range(self.tab_widget.count()):
            ...
            if isinstance(widget, EditorWidget):
                if self.editor_tabs_data.get(widget, {}).get('filepath') == norm_path:
    ...
    def _action_save_as(self) -> bool:
        ...
        new_path = self.file_handler.save_file_content(
            current_path, editor.get_text(), save_as=True
        )
        if new_path:
        ...
        for i in range(self.tab_widget.count()):
            ...
            if isinstance(editor, EditorWidget) and editor in self.editor_tabs_data:
                ...
                if is_dirty:
                    filepath = self.editor_tabs_data[editor].get('filepath')
                    if filepath:
                        if self.file_handler.save_file_content(filepath, editor.get_text()):
                            self._update_tab_info(editor, filepath)
        self._update_window_title()
    ...
    def _action_close_tab_by_index(self, index: int):
        ...
        is_dirty = (hash(widget.get_text()) !=
                    self.editor_tabs_data.get(widget, {})['original_hash'])
        if is_dirty:
            reply = QMessageBox.question(
            ...
            if reply == QMessageBox.StandardButton.Cancel:
                return
            if reply == QMessageBox.StandardButton.Save and \
               not self._action_save_file(widget=widget):
                return
    ...
    def closeEvent(self, event):
        ...
        unsaved_tabs = [
            ...
            if isinstance(self.tab_widget.widget(i), EditorWidget) and
            (hash(self.tab_widget.widget(i).get_text()) !=
             self.editor_tabs_data[self.tab_widget.widget(i)]['original_hash'])
        ]
    ...
    def _goto_definition_result(self, filepath: str, line: int, col: int):
        ...
        for i in range(self.tab_widget.count()):
            editor = self.tab_widget.widget(i)
            if isinstance(editor, EditorWidget):
                editor_path = self.editor_tabs_data.get(editor, {}).get('filepath')
                if editor_path == norm_path:
                    self.tab_widget.setCurrentIndex(i)
...
- L192 (E501) No message available
...
- L247 (E501) No message available
...
- L279 (E501) No message available

- L287 (E501) No message available

- L296 (E501) No message available

- L331 (E501) No message available
...
- L654 (E501) No message available

- L692 (E501) No message available

- L712 (E501) No message available

- L745 (E501) No message available

- L769 (E501) No message available

- L800 (E501) No message available

- L816 (E501) No message available

- L889 (E501) No message available

- L897 (E501) No message available

- L915 (E501) No message available

- L920 (E303) No message available

- L975 (E501) No message available

- L995 (E501) No message available

- L1011 (E501) No message available

- L1022 (E501) No message available

- L1057 (W292) No message available
...
class PreferencesDialog(QDialog):
    ...
    def _connect_global_signals(self):
        ...
        self.github_manager.auth_polling_lapsed.connect(
            self._on_auth_polling_lapsed
        )
    ...
    def _load_settings_into_dialog(self):
        # Block signals to prevent them firing during programmatic changes
        widgets_to_block = self.findChildren(
            (QComboBox, QSpinBox, QCheckBox, QFontComboBox, QLineEdit)
        )
        ...
        self.font_family_combo.setCurrentFont(
            QFont(settings_manager.get("font_family"))
        )
        self.font_size_spinbox.setValue(settings_manager.get("font_size"))
        self.show_line_numbers_checkbox.setChecked(
            settings_manager.get("show_line_numbers")
        )
        self.word_wrap_checkbox.setChecked(settings_manager.get("word_wrap"))
        self.show_indent_guides_checkbox.setChecked(
            settings_manager.get("show_indentation_guides")
        )
        self.indent_style_combo.setCurrentText(
            settings_manager.get("indent_style").capitalize()
        )
        self.indent_width_spinbox.setValue(settings_manager.get("indent_width"))
        self.auto_save_checkbox.setChecked(
            settings_manager.get("auto_save_enabled")
        )
        self.auto_save_delay_spinbox.setValue(
            settings_manager.get("auto_save_delay_seconds")
        )
        self.max_recent_files_spinbox.setValue(
            settings_manager.get("max_recent_files")
        )
        self.python_path_edit.setText(
            settings_manager.get("python_interpreter_path", sys.executable)
        )
        ...
        self.cleanup_build_checkbox.setChecked(
            settings_manager.get("cleanup_after_build", True)
        )
        self.staged_repos = [r.copy() for r in
                             settings_manager.get("source_control_repos", [])]
        self.staged_active_repo_id = settings_manager.get("active_update_repo_id")
        self._populate_repo_list()
        self.plugins_repo_edit.setText(
    ...
    def apply_settings(self):
        ...
        # Handle system-specific changes
        if sys.platform == "win32":
            orig_startup = self.original_settings.get("run_in_background", False)
            new_startup = self.run_in_background_checkbox.isChecked()
            if new_startup != orig_startup:
                self._manage_startup_shortcut(new_startup)

        # Handle Git config changes
        new_name = self.git_user_name_edit.text().strip()
        new_email = self.git_user_email_edit.text().strip()
        if (new_name != self.original_git_config.get('name') or
                new_email != self.original_git_config.get('email')):
    ...
    def reject(self):
        ...
        if self.theme_combo.currentData() != self.original_settings.get("last_theme_id"):
            self.theme_changed_signal.emit(
                self.original_settings.get("last_theme_id")
            )
        super().reject()
    ...
    def _repopulate_theme_combo(self):
        ...
        if (index := self.theme_combo.findData(current_id)) != -1:
    ...
    def _open_theme_editor_dialog(self):
        if not self.theme_editor_dialog_instance:
            ...
            self.theme_editor_dialog_instance.custom_themes_changed.connect(
                self._repopulate_theme_combo
            )
    ...
    def _create_run_tab(self):
        ...
        info = QLabel(
            "This interpreter is used for running scripts (F5) and code analysis."
        )
    ...
    def _browse_for_python(self):
        filter_str = "Executables (*.exe)" if sys.platform == "win32" else \
            "All Files (*)"
        path, _ = QFileDialog.getOpenFileName(
            self, "Select Python Interpreter", "", filter_str
        )
        if path:
    ...
    def _create_system_tab(self):
        ...
        self.run_in_background_checkbox = QCheckBox(
            "Launch at startup and run in background"
        )
        if sys.platform != "win32":
        ...
        info = QLabel(
            "Adds a tray icon that starts with Windows, allowing "
            "PuffinPyEditor to open faster."
        )
        info.setWordWrap(True)
        info.setStyleSheet("font-size: 9pt; color: grey;")
    ...
    def _manage_startup_shortcut(self, create: bool):
        ...
        try:
            if create:
                if not getattr(sys, 'frozen', False):
                    QMessageBox.warning(self, "Developer Mode",
                                        "Startup feature only works in a "
                                        "packaged application.")
                    self.run_in_background_checkbox.setChecked(False)
                    return
                ...
                if not os.path.exists(target_path):
                    QMessageBox.warning(
                        self, "File Not Found",
                        f"Could not find PuffinPyTray.exe in {install_dir}"
                    )
                    self.run_in_background_checkbox.setChecked(False)
                    return
                with winshell.shortcut(shortcut_path) as shortcut:
                    shortcut.path = target_path
                    shortcut.description = "PuffinPyEditor Background App"
            ...
            QMessageBox.critical(
                self, "Error", f"Could not modify startup shortcut.\n{e}"
            )
            self.run_in_background_checkbox.setChecked(False)
    ...
    def _create_source_control_tab(self):
        ...
        self.nsis_path_edit.setPlaceholderText(
            "e.g., C:\\Program Files (x86)\\NSIS\\makensis.exe"
        )
        ...
        self.cleanup_build_checkbox = QCheckBox(
            "Automatically clean up temporary build files"
        )
        self.cleanup_build_checkbox.setToolTip(
            "Deletes the 'build/' folder after a successful "
            "installer creation to save space."
        )
        ...
        branch_fix_button.clicked.connect(
            self.git_manager.set_default_branch_to_main
        )

    def _browse_for_nsis(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Select makensis.exe (Command-Line Version)",
    ...
    def _create_repo_management_widgets(self) -> tuple[QWidget, QWidget]:
        ...
        add_repo_btn = QPushButton("Add Existing...")
        ...
        self.repo_active_checkbox = QCheckBox(
            "Set as Primary (for updates & publishing)"
        )
        repo_form_layout.addRow("Friendly Name:", self.repo_name_edit)
        repo_form_layout.addRow("Owner (e.g., 'Stelliro'):", self.repo_owner_edit)
        repo_form_layout.addRow("Repository (e.g., 'PuffinPyEditor'):",
                                self.repo_repo_edit)
        ...
        self.repo_list_widget.currentItemChanged.connect(
            self._on_repo_selection_changed
        )
        self.repo_active_checkbox.toggled.connect(
            self._on_active_checkbox_toggled
        )
    ...
    def _action_create_repo(self):
        ...
        if not (ok and name):
            return
        description, _ = QInputDialog.getText(
            self, "Create New Repository", "Description (optional):"
        )
        is_private = QMessageBox.question(
            self, "Visibility", "Make this repository private?",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
            QMessageBox.StandardButton.No
        ) == QMessageBox.StandardButton.Yes
        self.github_manager.create_repo(name, description, is_private)
        QMessageBox.information(
            self, "In Progress", f"Attempting to create '{name}' on GitHub..."
        )
    ...
    def _update_auth_status(self):
        ...
        self.auth_status_label.setText(
            f"Logged in as: <b>{user}</b>" if is_logged_in else "Not logged in."
        )
    ...
    def _on_device_code_ready(self, data: dict):
        self.auth_dialog = AuthDialog(
            data['user_code'], data['verification_uri'], self
        )
    ...
    def _on_auth_successful(self, username: str):
        ...
        if not self.git_user_email_edit.text():
            self.git_user_email_edit.setPlaceholderText(
                f"e.g., {username}@users.noreply.github.com"
            )
    ...
    def _on_auth_polling_lapsed(self):
        if self.auth_dialog:
            self.auth_dialog.reject()
        QMessageBox.warning(
            self, "Login Expired", "The login code has expired. Please try again."
        )
        self._update_auth_status()

    def _on_repo_selection_changed(
        self, current_item: Optional[QListWidgetItem],
        ...
        if not current_item:
            ...
            return
        self._load_staged_to_repo_form(
    ...
    def _save_repo_form_to_staged(self):
        if not self.current_repo_id_in_form:
            return
        repo = next((r for r in self.staged_repos if
                     r.get("id") == self.current_repo_id_in_form), None)
        if repo:
            repo.update({
                'name': self.repo_name_edit.text().strip(),
                'owner': self.repo_owner_edit.text().strip(),
                'repo': self.repo_repo_edit.text().strip()
            })

    def _load_staged_to_repo_form(self, repo_id: str):
        repo_data = next(
            (r for r in self.staged_repos if r.get("id") == repo_id), None
        )
        if not repo_data:
            self._clear_repo_form()
            return
        ...
        widgets_to_block = [self.repo_name_edit, self.repo_owner_edit,
                            self.repo_repo_edit, self.repo_active_checkbox]
    ...
    def _action_add_repo(self):
        ...
        self.staged_repos.append(
            {"id": new_id, "name": "New Repository", "owner": "", "repo": ""}
        )
    ...
    def _action_remove_repo(self):
        ...
        repo_name = (f"'{repo.get('owner')}/{repo.get('repo')}'" if repo
                     else f"'{current_item.text()}'")

        msg_box = QMessageBox(self)
        msg_box.setIcon(QMessageBox.Icon.Warning)
        msg_box.setWindowTitle("Confirm Remove")
        msg_box.setTextFormat(Qt.TextFormat.RichText)
        msg_box.setText(f"Are you sure you want to remove the repository "
                        f"{repo_name} from PuffinPyEditor's list?")
        msg_box.setInformativeText(
            "<b>This will NOT delete the repository from GitHub.</b> "
        ...
        if msg_box.exec() == QMessageBox.StandardButton.Yes:
            self.staged_repos = [r for r in self.staged_repos
                                 if r.get("id") != repo_id]
    ...
    def _populate_repo_list(self, select_repo_id: Optional[str] = None):
        ...
        for repo in sorted(self.staged_repos, key=lambda r: r.get('name', '').lower()):
            ...
            if repo.get("id") == self.staged_active_repo_id:
                item.setIcon(qta.icon('fa5s.star', color='gold'))
            self.repo_list_widget.addItem(item)
    ...
    def _create_plugins_tab(self):
        ...
        self.uninstall_plugin_button.setIcon(
            qta.icon('fa5s.trash-alt', color='crimson')
        )
        self.uninstall_plugin_button.setEnabled(False)
        installed_layout.addWidget(self.installed_plugins_list)
        installed_layout.addWidget(self.uninstall_plugin_button, 0,
        ...
        self.install_remote_button.clicked.connect(
            self._install_selected_remote_plugin
        )
        self.uninstall_plugin_button.clicked.connect(
            self._uninstall_selected_plugin
        )
        self.installed_plugins_list.currentItemChanged.connect(
            self._on_installed_plugin_selected
        )
        self.remote_plugins_list.currentItemChanged.connect(
            self._on_remote_plugin_selected
        )

    def _populate_installed_plugins_list(self):
        self.installed_plugins_list.clear()
        # FIX: Use get_all_plugins() and adapt to the Plugin object structure
        plugins = sorted(self.plugin_manager.get_all_plugins(), key=lambda p: p.name.lower())
        for plugin in plugins:
            name_suffix = " (Core Tool)" if plugin.is_core else ""
            item_text = f"{plugin.name} v{plugin.version}{name_suffix}"

            list_item = QListWidgetItem(item_text)
            tooltip = (f"ID: {plugin.id}\n"
                       f"Status: {plugin.status_reason}\n"
                       f"Description: {plugin.manifest.get('description', 'N/A')}")
            list_item.setToolTip(tooltip)

            # FIX: Create a dict from the plugin object for UserRole data, as
            # other parts of the dialog expect this.
            plugin_data = {
                'id': plugin.id,
                'name': plugin.name,
                'is_core': plugin.is_core,
                'description': plugin.manifest.get('description', 'N/A')
            }
            list_item.setData(Qt.ItemDataRole.UserRole, plugin_data)

            if plugin.is_core:
                list_item.setForeground(QColor("grey"))

            if not plugin.enabled:
                list_item.setForeground(QColor("darkgrey"))
                list_item.setFont(QFont(list_item.font().family(), -1, -1, True))
    ...
    def _on_installed_plugin_selected(self, current_item: QListWidgetItem):
        ...
        self.uninstall_plugin_button.setToolTip(
            "Core plugins cannot be uninstalled." if is_core
            else "Uninstall the selected plugin."
        )
    ...
    def _fetch_remote_plugins(self):
        ...
        if not repo_path or '/' not in repo_path:
            QMessageBox.warning(
                self, "Invalid Repo", "Enter a valid GitHub repo (user/repo)."
            )
            return
    ...
    def _install_plugin_from_url(self, url: str):
        if not url:
            QMessageBox.critical(self, "Error", "Plugin has no download URL.")
            return
        try:
            ...
            success, message = self.plugin_manager.install_plugin_from_zip(
                temp_zip_path
            )
            if success:
                QMessageBox.information(self, "Success", message)
    ...
    def _install_plugin_from_file(self):
        filepath, _ = QFileDialog.getOpenFileName(
            self, "Select Plugin Archive", "", "ZIP Files (*.zip)"
        )
        if not filepath:
            return
    ...
    def _uninstall_selected_plugin(self):
        ...
        if not current_item:
            QMessageBox.warning(
                self, "No Selection", "Please select a plugin to uninstall."
            )
            return
        ...
        if plugin_data.get("is_core", False):
            QMessageBox.information(
                ...
                "This is a core plugin and cannot be uninstalled."
            )
            return

        if QMessageBox.question(self, "Confirm Uninstall",
                                f"Uninstall '{current_item.text()}'?") == \
                QMessageBox.StandardButton.Yes:
