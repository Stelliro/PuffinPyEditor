...
## Export Timestamp: 2025-07-01T17:49:13.702872
---
...
class FileSystemListView(QWidget):
    def __init__(self, puffin_api: PuffinPluginAPI, path: str, parent: None = None):
        super().__init__(parent)
        self.api = puffin_api
        self.project_path = path  # The root path for this view
        ...
        self.tree_widget.setIndentation(15) # ADDED: Set indentation for visual guidelines
    ...
    def refresh(self):
        """This can be called externally to refresh the view."""
        self.tree_widget.clear()
        self._populate_node(self.tree_widget.invisibleRootItem(), self.project_path)

    def _populate_node(self, parent_item, path):
        if not os.path.isdir(path):
            return

        # Clear existing dummy item if any
        if parent_item.childCount() == 1 and not parent_item.child(0).text(0):
        ...
        try:
            ...
            for entry in entries:
                ...
                item.setData(0, Qt.ItemDataRole.UserRole, {'path': entry.path, 'is_dir': is_dir})
                
                if is_dir:
                    # Add a dummy child to make the expander arrow appear
                    dummy_child = QTreeWidgetItem(item)
    ...
    def on_item_collapsed(self, item):
        # Clear all children and add a single dummy node back
        item.takeChildren()
        dummy_child = QTreeWidgetItem(item)
        item.addChild(dummy_child)
    ...
    def on_item_double_clicked(self, item, column):
        ...
        if data.get('is_dir'):
            ...
            path = data.get('path')
            if path:
                self.main_window._action_open_file(path)
    ...
    def show_context_menu(self, position):
        ...
        if item:
            ...
            # Right-clicked on empty space, use the root project path
    ...
    # --- Action Methods for Context Menu ---
    ...
    def _action_rename(self, old_path: str):
        ...
        if ok and new_name and new_name != old_name:
            success, message = self.file_handler.rename_item(old_path, new_name)
            if not success:
                QMessageBox.warning(self, "Error", message)
            else:
                self.refresh()
...
class ListViewExplorerPlugin:
    ...
    def refresh_project_views(self):
        ...
        # MODIFIED: Changed .path to .project_path
        current_paths = {
            self.tab_widget.widget(i).project_path for i in range(self.tab_widget.count())
            if hasattr(self.tab_widget.widget(i), 'project_path')
        }

        for i in range(self.tab_widget.count() - 1, -1, -1):
            widget = self.tab_widget.widget(i)
            # MODIFIED: Changed .path to .project_path
            if hasattr(widget, 'project_path') and widget.project_path not in open_projects:
        ...
        for i in range(self.tab_widget.count()):
            widget = self.tab_widget.widget(i)
            # MODIFIED: Changed .path to .project_path
            if hasattr(widget, 'project_path') and widget.project_path == active_path:
    ...
    def _handle_tab_close(self, index: int):
        ...
        # MODIFIED: Changed .path to .project_path
        if hasattr(view, 'project_path'):
            self.api.get_main_window()._action_close_project(view.project_path)
    ...
    def _on_active_project_changed(self, index: int):
        if index != -1:
            ...
            # MODIFIED: Changed .path to .project_path
            if hasattr(view, 'project_path'):
                self.project_manager.set_active_project(view.project_path)
...
class SourceControlUIPlugin:
    ...
    def _connect_signals(self):
        ...
        # Get the list_view_explorer plugin instance, which is now a dependency
        list_view_explorer_plugin = plugin_manager.plugins.get('list_view_explorer')

        if list_view_explorer_plugin and list_view_explorer_plugin.instance and hasattr(list_view_explorer_plugin.instance, 'tab_widget'):
            # Connect to the tab_widget from the list view explorer plugin
            list_view_explorer_plugin.instance.tab_widget.currentChanged.connect(
                self.source_control_panel.refresh_all_projects)
            self.api.log_info("SourceControlUI connected to ListViewExplorer's tabs.")
        else:
            self.api.log_warning("SourceControlUI: Could not find List View Explorer plugin or its tab widget to connect signals.")
