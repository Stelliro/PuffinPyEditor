...
## Export Timestamp: 2025-07-05T17:35:13.441620
---
...
class FileSystemListView(QWidget):
    ...
    def refresh(self):
        ...
        expanded_paths = set(self.get_expanded_paths())
        ...
        self._restore_expanded_state(self.tree_widget.invisibleRootItem(), expanded_paths)
        if current_path:
            self._select_and_scroll_to_path(current_path)

        get_git_statuses_for_root.cache_clear()

    def _restore_expanded_state(self, parent_item, expanded_paths):
        """Recursively expand items that were previously expanded."""
        for i in range(parent_item.childCount()):
            child = parent_item.child(i)
            data = child.data(0, Qt.ItemDataRole.UserRole)
            path = data.get('path') if data else None

            if path and os.path.normpath(path) in expanded_paths:
                child.setExpanded(True)
                # After expanding, its children are populated, so we recurse
...
class PythonMiniMap(MiniMapWidget):
    """A specialized minimap that functions as a clickable, scrollable code outline."""
    ...
    def _load_icons(self):
        ...
        # MODIFIED: Using fixed, vibrant colors for better visual distinction in the guide,
        # inspired by popular IDE themes.
        class_icon_color = "#E5C07B"  # A soft, distinct gold for classes
        def_icon_color = "#61AFEF"   # A clear, readable blue for functions

        self.class_icon = qta.icon('fa5s.cubes', color=class_icon_color)
        self.def_icon = qta.icon('fa5s.cube', color=def_icon_color)

    def _get_indent(self, text: str) -> int: return len(text) - len(text.lstrip())
    ...
    def _precompute_structure(self) -> List[Tuple[int, int, str, str]]:
        ...
        for i in range(doc.blockCount()):
            ...
            if class_match: structure.append((i, self._get_indent(text), "class", class_match.group(1)))
            elif def_match: structure.append((i, self._get_indent(text), "def", def_match.group(1)))
        return structure

    def _format_name(self, name: str) -> str: return name.lstrip('_').replace('_', ' ').title()
        ...
        if event.button() == Qt.MouseButton.LeftButton:
            for i, (region_rect, line_num) in enumerate(self.clickable_regions):
                if region_rect.contains(event.position()):
                    ...
                    self.editor.setTextCursor(cursor); self.editor.centerCursor(); return
            v_scroll = self.editor.verticalScrollBar()
    ...
    def mouseMoveEvent(self, event: QMouseEvent):
        ...
        for i, (region_rect, _) in enumerate(self.clickable_regions):
            if region_rect.contains(event.position()): new_hover_index = i; break
        if new_hover_index != self.hovered_region_index:
            self.hovered_region_index = new_hover_index; self.update()
        super().mouseMoveEvent(event)
    ...
    def _draw_structure_items(self, painter, colors, structure):
        ...
        label_font = self.editor.font(); label_font.setPointSizeF(label_font.pointSizeF() * 0.9)
        painter.setFont(label_font); font_metrics = painter.fontMetrics()
        v_scroll = self.editor.verticalScrollBar()
    ...
    def _draw_viewport_indicator(self, painter, colors):
        ...
        try: visible_line_count = self.editor.viewport().height() // self.editor.fontMetrics().height()
        except ZeroDivisionError: visible_line_count = 0
...
class MainWindow(QMainWindow):
    ...
    def _action_close_tab_by_index(self, index):
        ...
        if dummy_close_event.isAccepted():
            # Check if widget is still valid before removing
    ...
    def _close_widget_safely(self, widget, event):
        if not isinstance(widget, (EditorWidget, QWidget)) or (isinstance(widget, QLabel) and widget.objectName() == "PlaceholderLabel"):
            event.accept()
            return
        ...
        if is_editor_widget and self._is_editor_modified(widget):
            ...
            else: # Fallback for floating windows
                tab_name = os.path.basename(self.editor_tabs_data.get(widget, {}).get('filepath', 'Untitled'))
