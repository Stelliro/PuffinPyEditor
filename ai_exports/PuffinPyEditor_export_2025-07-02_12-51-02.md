...
## Export Timestamp: 2025-07-02T12:51:02.835156
---

## AI Instructions
You are a senior developer's assistant. Your task is to analyze the project's structure and conventions to generate boilerplate code for new features. This helps developers start new tasks quickly without writing repetitive setup code.
...
class FindReplacePlugin:
    ...
    def toggle_find_panel(self):
        ...
        if self.find_panel.isVisible() and self.find_panel.parent() is editor:
            ...
            # This now correctly inserts into the EditorWidget's main QVBoxLayout
...
class BreakpointArea(QWidget):
    ...
    def mouseMoveEvent(self, event: QMouseEvent):
        try:
            ...
            # FIX: Use the text area's viewport coordinates for a stable hit-test
    ...
    def paintEvent(self, event) -> None:
        try:
            ...
            top = text_area.blockBoundingGeometry(block).translated(offset).top()
            bottom = top + text_area.blockBoundingRect(block).height()
            ...
            while block.isValid() and top <= event.rect().bottom():
                if block.isVisible() and bottom >= event.rect().top():
                    ...
                    dot_y = int(top + (text_area.blockBoundingRect(block).height() - radius * 2) / 2)
            ...
            log.critical(f"CRASH in BreakpointArea.paintEvent: {e}", exc_info=True)
...
class LineNumberArea(QWidget):
    """A widget that acts as a canvas for painting line numbers."""
    def __init__(self, editor_widget):
        super().__init__(editor_widget)
        self.editor_widget = editor_widget
...
class EditorWidget(QWidget):
    ...
    def __init__(self, completion_manager=None, parent=None):
        ...
        # This is the main layout. The FindPanel will be inserted at index 0.
        self.main_layout = QVBoxLayout(self)
        self.main_layout.setContentsMargins(0, 0, 0, 0)
        self.main_layout.setSpacing(0)

        # A container for the editor and its sidebars (gutters)
        self.editor_area_widget = QWidget()
        editor_hbox = QHBoxLayout(self.editor_area_widget)
        ...
        self.main_layout.addWidget(self.editor_area_widget)
    ...
    def line_number_area_paint_event(self, event):
        try:
            ...
            top = self.text_area.blockBoundingGeometry(block).translated(offset).top()
            ...
            log.critical(f"CRASH in line_number_area_paint_event: {e}", exc_info=True)
...
class MainWindow(QMainWindow):
    ...
    def _add_new_tab(self, filepath=None, content="", is_placeholder=False):
        log.debug(f"_add_new_tab called. Filepath: {filepath}, Placeholder: {is_placeholder}")
        if self.tab_widget.count() == 1 and isinstance(self.tab_widget.widget(0), QLabel):
            log.debug("Removing placeholder tab.")
        ...
        if is_placeholder:
            ...
            self.tab_widget.setTabsClosable(False);
            log.debug("Placeholder tab added.")
            return

        try:
            self.tab_widget.setTabsClosable(True)
            log.debug("Creating EditorWidget...")
            editor = EditorWidget(self.completion_manager, self)
            log.debug("EditorWidget created.")

            highlighter_class = None
            if filepath:
                _, file_ext = os.path.splitext(filepath)
                file_ext = file_ext.lower()
                log.debug(f"File extension is '{file_ext}'. Checking for highlighter.")
                if file_ext in self.puffin_api.highlighter_map:
                    highlighter_class = self.puffin_api.highlighter_map[file_ext]
                    log.debug(f"Found plugin highlighter: {highlighter_class.__name__}")
                elif file_ext == '.py':
                    from .widgets.syntax_highlighter import PythonSyntaxHighlighter
                    highlighter_class = PythonSyntaxHighlighter
                    log.debug("Using built-in PythonSyntaxHighlighter.")

            log.debug(f"Setting highlighter on editor: {highlighter_class}")
            editor.set_highlighter(highlighter_class)

            log.debug(f"Setting filepath on editor: {filepath}")
            editor.set_filepath(filepath);
            log.debug("Setting text on editor...")
            editor.set_text(content)
            log.debug("Text set. Connecting signals.")
            ...
            log.debug(f"Adding tab with name: {name}")
            ...
            self._update_window_title()
            log.debug(f"Tab '{name}' added successfully.")
        except Exception as e:
            log.critical(f"CRASH during _add_new_tab: {e}", exc_info=True)
            QMessageBox.critical(self, "Fatal Error", f"A critical error occurred while creating a new tab:\n\n{e}")

    def _action_open_file(self, filepath: str, content: str = None):
        log.info(f"Action to open file triggered: {filepath}")
        norm_path = os.path.normpath(filepath)
        file_ext = os.path.splitext(norm_path)[1].lower()

        log.debug(f"Normalized path: {norm_path}, extension: {file_ext}")
        if handler := self.file_open_handlers.get(file_ext):
            log.info(f"Found custom handler for '{file_ext}', delegating opening.")
            handler(norm_path)
        ...
        for i in range(self.tab_widget.count()):
            ...
            if isinstance(widget, EditorWidget) and self.editor_tabs_data.get(widget, {}).get('filepath') == norm_path:
                log.info(f"File '{filepath}' is already open. Switching to tab {i}.")
                self.tab_widget.setCurrentIndex(i);
                return

        if content is None:
            try:
                log.debug(f"Reading content from file: {norm_path}")
                with open(norm_path, 'r', encoding='utf-8', errors='replace') as f:
                    content = f.read()
                log.debug("File content read successfully.")
            except Exception as e:
                log.error(f"Failed to read file content for {norm_path}: {e}", exc_info=True)
                QMessageBox.critical(self, "Error Opening File", f"Could not read file: {e}");
                return

        self._add_new_tab(norm_path, content)
        self.file_handler._add_to_recent_files(norm_path)
        log.info(f"File '{filepath}' opened in a new tab.")
